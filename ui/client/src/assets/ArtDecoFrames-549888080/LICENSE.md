...
The PDF held locally at `ui/attached_assets/AdobeStock-549888080-Invoice.PDF`
...
`AdobeStock-549888080-ArtDecoFrames.ai` using the repo's
`extract-vector-grid` skill (`.claude/skills/extract-vector-grid/`). The
source vector paths are preserved verbatim; only isolation, translation,
and viewBox are derived. See `manifest.json` for the per-frame mapping.
