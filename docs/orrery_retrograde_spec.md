...
13. `event_source_kind` enum migration — add `'retrograde'` variant. Current values: `'apex', 'resolver', 'narrator', 'bleed', 'authored'`. Using `'authored'` for retrograde rows would conflate wizard-time LLM generation with hand-authored content. Migration follows the pattern in migration 036 (`ALTER TYPE ... ADD VALUE IF NOT EXISTS`). *R6 dry-run expansion now carries this as an explicit commit blocker instead of writing.*
...
15. Skald-as-weaver expansion contract for Stage R6 (paired with #9). *Partially addressed by `retrograde_expansion.py`: distinct prompt, output schema, live-call CLI, selected-seed accounting, and non-mutating row-shaped event/tag/relationship plans exist. Remaining work is persistence: pre-game event anchoring, idempotency, transaction semantics, and MEMNON visibility boundary.*
