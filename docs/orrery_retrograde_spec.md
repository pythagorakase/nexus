...
- **R1. Vocabulary table.** Enumerate the seed-eligible primitives: event types, tags, relationship types, place affordances already in the Orrery registries. Retrograde generates *only* in existing vocabulary so output is substrate-legal by construction. New vocabulary is out of scope for a generation run.
...
9. Skald-as-narrator structured-output schema for `new_entities` declarations (kind, name, summary, optional MV-tag hints).
...
11. Audit-pass design for catching Skald-as-narrator declarations missed in prose — pure rule-based (regex / NER + DB existence check), no LLM inference. Confidence threshold, false-positive tolerance.
