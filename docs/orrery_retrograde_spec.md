...
**Status:** Design spec, deliberately compressed. Phase A0 has a non-mutating dry-run packet surface, Phase A1 adds a non-mutating R4/R5 seed-generation request contract, R6 can make a live Skald-as-weaver expansion call, and the persistence surface can dry-run or apply row-shaped expansion output into canonical Orrery tables when blockers are clear. As of M4 the whole pipeline fires automatically inside the new-story wizard's ready -> narrative transition (`nexus/agents/orrery/retrograde_orchestrator.py`, composed by `nexus.api.new_story_flow.perform_transition_with_retrograde`); the stage CLI verbs remain as calibration surfaces over the same core. M9 separates generated event summaries from played-story continuity while retaining the synthetic prologue FK anchor. Remaining implementation decisions are flagged inline as **[OPEN]**.
...
- **Substrate:** reuse Orrery's `world_events`, `entity_tags`, relationship tables, vocabulary registries. Wizard-time generated history anchors to one synthetic finalized prologue `narrative_chunks` row rather than weakening `world_events.tick_chunk_id`. Event prose lives in `retrograde_summaries`, keyed one-to-one to `world_events`; it never advances played-story continuity. `world_events` rows use `source = 'retrograde'`; generated `entity_tags` / `entity_pair_tags` rows use `source_kind = 'retrograde'`.
- **MEMNON:** retrieves generated history through the same semantic/text-search contract as play-generated history while preserving its distinct storage identity. The storyteller receives relevant history, not storage provenance.
...
3. Pre-game `tick_chunk_id` FK resolution — **resolved (M9 boundary clarified).** `world_events.tick_chunk_id` remains `bigint NOT NULL REFERENCES narrative_chunks(id)`. Retrograde uses one synthetic finalized prologue `narrative_chunks` row, with matching `chunk_metadata`, as the wizard-time anchor. It is the only Retrograde auxiliary row permitted in `narrative_chunks` and is excluded by the canonical playable-narrative predicate from latest, outline, adjacent, episode, direct-read, resume, and checkpoint-cadence surfaces. Its raw PK remains a storage anchor only; readers preserve resulting id gaps and label scenes from slug/scene/order metadata.
...
9. Skald-as-narrator structured-output schema for `new_entities` declarations — **resolved (M8, 2026-06-10; pair-hint application repaired in #477 Stage 2a).** `NewEntityDeclaration` in `nexus/agents/logon/apex_schema.py`: kind (`character`/`place`/`faction`), name, one-line summary, optional registered `tag_hints` (single-entity) and `pair_tag_hints` (tag + other-endpoint name + declared-entity role). The field rides `StorytellerResponseMinimal/Standard/Extended`, through the incubator (`incubator.new_entities`, migration 062), and is processed at commit time by `enqueue_declared_entity_maturations` (`nexus/agents/orrery/retrograde_maturation.py`): hints are validated against the live `tags` / `pair_tags` registries — unregistered, deprecated, or kind-incompatible names fail the commit loudly — stub rows are created for declared entities absent from the database (Skald-curated summary becomes the stub summary; tag hints become the minimum viable tag set via the standard bestowal path), and pair-tag hints are applied through the standard ledgered pair-tag writers with `source_kind = 'skald_inline'` while also feeding the maturation packet as prompt material. The declaration duty is documented sparingly in `prompts/storyteller_core.md` (declare only entities likely to recur).
10. Async job-queue infrastructure for runtime maturation — **resolved for the fire-and-forget core (M8, storage identity updated M9).** Durable queue `orrery_maturation_jobs` (migration 062) mirrors the narration outbox's lease/retry discipline: enqueued inside the chunk-commit transaction (atomic with the chunk — the outbox boundary), drained by the post-commit Orrery worker (`drain_maturation_jobs_sync`, wired into `process_orrery_outbox_sync`). Per job the worker composes existing pipeline functions: scoped single-entity packet (`build_runtime_maturation_packet`, reusing the wizard request builder with tighter budgets) → R4/R5 seed generation → R6 expansion → `build_retrograde_persistence_plan` execute → dedicated Retrograde-summary embedding. Event refs are namespaced per job so per-slot `payload.retrograde_event_ref` idempotency keys never collide; the unique index on `entity_id` plus an already-connected world-events guard make maturation idempotent per entity (a matured entity never re-matures). Failed jobs record `last_error` and requeue with a delay until the attempt cap, then stay `failed` and visible; a job whose manifest already records persisted rows resumes at the embedding step instead of regenerating history. Knobs in `nexus.toml [orrery.retrograde.maturation]` (enabled, budget, drain/attempt caps, seed budgets, model ref). **Post-1.0:** branch-parallel speculative execution (maturing A/B/C options during the turn-composition pause, persistence regardless of branch chosen) — v1 matures on the committed branch only.
...
14. `CommitOrreryTick` bypass contract for wizard-time Retrograde — **resolved for storage and retrieval (M9).** Execute mode writes one `retrograde_summaries` row per persisted Retrograde `world_events` row, carrying the Skald-woven event prose, chronology, recording anchor, and embedding lifecycle. Dimension-specific `retrograde_summary_embeddings_<dimensions>d` tables preserve vector identity without pretending generated history is a played chunk. Migration 078 validates every legacy marker/link pair, backfills canonical Retrograde events that predate legacy summary writes, preserves legacy summary and vector identities, rejects embedding stamps with no copyable vector, upgrades maturation manifests from chunk to summary identities, removes the old payload pseudo-link, and deletes only legacy event-summary narrative rows. It holds source writers under one runner-owned transaction and installs database guards that reject the retired marker, pseudo-link, and stale v0 or chunk-shaped maturation manifests after cutover; the synthetic prologue remains an unembedded FK anchor (decision 3). Retrieval unifies dedicated Retrograde summary hits with aged narrative hits, while recent/warm/player-facing story surfaces consume playable narrative only. Interval checkpoints fire on accepted playable ordinals and retain the actual sparse `narrative_chunks.id` as their replay anchor. **The wizard-time product boundary remains M4's atomic contract:** Retrograde persistence executes inside the same transaction as wizard transition writes (`perform_transition`'s `in_transaction` hook). If expansion succeeds but persistence is blocked, `RetrogradePersistenceBlockedError` aborts the transaction: no protagonist, places, history, or transition rows land; the wizard cache survives and the API surfaces the blocker list loudly. Post-commit embedding failure leaves canonical summary rows pending retry via their `embedding_generated_at` state. Wizard-time progress states (packet -> seed_candidates -> expansion -> persistence -> embedding -> done/failed) remain exposed at `GET /api/story/new/retrograde/status?slot=N`.
