...
- **R1. Vocabulary table.** Enumerate the seed-eligible primitives: event types, tags, relationship types, place affordances already in the Orrery registries. Retrograde generates *only* in existing vocabulary so output is substrate-legal by construction. New vocabulary is out of scope for a generation run. A first-pass enumerator exists at `nexus/agents/orrery/retrograde_vocabulary.py`; before full Retrograde implementation it should be reconciled against live DB registry rows so post-migration deltas such as migration 045's `claims` subject-kind extension and trait-compiler pair-tags are visible to prompts. Track this under the full Retrograde implementation issue (#300) before relying on the enumerator for Phase A bootstrap prompts.
...
- **R4. Seed generation (high-entropy, high-friction).** Over-generate candidate seeds — events, grudges, debts, secrets, vanished parties — that are *directionally connected* to the graph but NOT implied by it. This is where surprise is injected. Generate more than will be kept.

- **R5. Semi-constrained selection.** Filter/weight the candidate seeds by wizard choices (especially character traits) and the weird setting (below). Select a subset. **Discarded seeds cost only inference.**
