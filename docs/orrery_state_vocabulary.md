...
**Polypharmacy.** Multi-valued: different classes are distinct `tag_id`s and coexist (`intoxicated:stimulant` + `intoxicated:depressant` is a specific, recognizable state, and the suppression/impairment effects compose). Same-class re-dose is *not* a second row — the unique index forbids it — so the writer must **extend the open row's timer** on conflict. **Substrate prerequisite:** the additive-timer behavior requires both a stored `expires_at_world_time` column on `entity_tags` and a tag-template-declared `on_conflict_policy` for the writer to dispatch on (currently `ON CONFLICT DO NOTHING` at `tag_writer.py:268`). Tracked in #329, sibling to #328.
...
**Predicates fall out of the convention:** `has_tag("intoxicated:stimulant")` for the specific gate; `has_tag_prefix("intoxicated")` for "under any influence."
...
**Where clearance config lives.** Per-tag-template fields declare *which* event types clear *this* state — clearance is a property of the state, not of the event. Adding a new event type doesn't require enumerating every state it could clear; the existing `_clear_entity_tag_siblings` writer pattern (`nexus/agents/orrery/tag_writer.py:277`) already keys off tag category, not event identity. The event registry below lists *names referenced from the state side*; whether each currently exists in the `event_types` table is independent of whether any state has actually wired it up. A state whose clearance config names a nonexistent event type does not no-op — it applies and **never clears, silently** — so the event vocabulary cannot lag the state vocabulary.
...
1. **`under_active_pursuit` → inbound `hunting` pair-tag.** Live in three needs gates (SLEEP, SOCIALIZE, INTIMACY carry `NOT(...)` against it). The canonical category-error tag the whole refactor targets — it should be `hunting(X → char)`, inbound. The gates need rewriting to the inbound-pair-tag predicate once the `has_inbound_pair_tag` predicate ships. Tracked in **#318**; template-rewrite work, not vocabulary work.
