...
**Bleed offers a menu, never a decision.** Storyteller-time Bleed chooses deterministically from eligible narrated events. Output is a bounded list of N candidates (typically N ≤ 3), each annotated with a sensory channel (auditory, news fragment, secondhand mention, faction graffiti) and a thin perceptual descriptor — not the narrator's full prose.

**Hook point**: Bleed runs at the start of LORE Phase 5 (`assemble_context_payload`, `turn_cycle.py:489`). Its output populates `turn_context.bleed_menu`, which `assemble_context_payload` then reads when building the payload.
