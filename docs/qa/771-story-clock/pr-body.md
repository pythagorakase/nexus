...
```sh
PYTHONPATH=$PWD PGOPTIONS='-c default_transaction_read_only=on' NEXUS_GATEWAY_PORT=8015 NEXUS_API_URL=http://127.0.0.1:8015 $PY -m nexus.cli up --slot 1 --config artifacts/771/proof.nexus.toml
```
...
```sh
PYTHONPATH=$PWD NEXUS_GATEWAY_PORT=8015 NEXUS_API_URL=http://127.0.0.1:8015 $PY -m nexus.cli down --config artifacts/771/proof.nexus.toml
```
