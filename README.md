...
Our project aims to compensate for these weaknesses with one or more LLMs (currently planning for 2) and a modular, agentic collection of scripts that coordinate to:
...
## Current and Future System Specifications
...
### 01 User Input
1. Simple chat-like interface within Obsidian displays recent narrative in markdown format.
2. User inputs next passage.
- [X] custom plugin correctly displays chunks
...
- Implements event-based communication protocol
...
- Model: API call to Claude 3.5 or GPT-4o
...
### `memory/` Shared Memory System
    ...
    - `EntityRegistry`: SQLite integration for structured data
...
## AI Role Summary
- **Most Core Functions & Agents**: LLama 3 70B 4-5 bit
...
The framework uses a standardized communication protocol between agents based on the following key components:

### BaseAgent Class

The `BaseAgent` class in `agent_base.py` provides the foundation for all agent modules with:

- Standardized message sending and receiving methods
- Support for specialized action handlers via the `on_{action}` method naming convention
- Consistent response formatting with the `create_response` helper
- Automatic dispatching of messages to appropriate handlers

### Maestro Orchestrator

The `Maestro` class in `maestro.py` serves as the central orchestrator with:

- An `AgentRegistry` for managing agent instances
- Standardized message routing between agents
- Broadcast message capability
- Support for both direct and registry-based communication

### Real Agent Communication

The communication system has been fully implemented and tested with real agent instances:

- ✅ Direct agent-to-agent communication works with real agent instances
- ✅ Registry-based communication works for agent modules registered with Maestro
- ✅ Broadcast communication successfully delivers messages to multiple agents
- ✅ Multi-hop communication (agent1 -> agent2 -> agent3) is supported
- ✅ Specialized action handlers can be defined and used with `on_{action}` naming convention
- ✅ Responses are properly formatted and returned

### Communication Testing Status

All communication test scenarios are now passing:
...
The `test_agent_communication.py` script demonstrates real agent communication without mocks:

```bash
# Run the communication tests
python test_agent_communication.py

# Run just the real agent communication test through prove.py
python prove.py --test 6
```

Key test scenarios:

1. **Direct Communication**: Agent1 sends a request to Agent2, which processes it and returns a response
2. **Multi-hop Communication**: Agent1 sends a request to Agent2, which forwards it to Agent3 for processing, and then returns the combined result
3. **Broadcast Communication**: Maestro broadcasts a message to all registered agents

## Next Steps

- Complete the registry-based communication testing
- Implement communication middleware for logging, validation, and security
- Add more specialized agent types with domain-specific handlers