...
def analyze_temporal_intent(query_text: str) -> float:
    ...
    for signal, weight in early_signals.items():
        if signal in query_lower or re.search(
            r"\b" + re.escape(signal) + r"\b", query_lower
        ):
            temporal_score = min(
                temporal_score, weight
            )  # Take the earliest signal found
            signals_found += 1
    ...
    for signal, weight in recent_signals.items():
        if signal in query_lower or re.search(
            ...
            temporal_score = max(
                temporal_score, weight
            )  # Take the most recent signal found
            signals_found += 1
    ...
    for term, bias in event_terms.items():
        if term in query_lower or re.search(
            r"\b" + re.escape(term) + r"\b", query_lower
        ):
...
def apply_continuous_temporal_boost(
    ...
    intent_strength = (
        abs(query_temporal_intent - 0.5) * 2
    )  # 0 at neutral, 1 at extremes
    ...
    adjusted_score = (
        base_score * (1.0 - temporal_boost_factor) + match_score * temporal_boost_factor
    )
    ...
    if abs(adjusted_score - base_score) > 0.2:
        logger.debug(
            f"Significant temporal adjustment: {base_score:.4f} -> {adjusted_score:.4f} "
...
def result_temporal_anchor(result: Dict[str, Any]) -> Optional[int]:
    """Return the narrative chronology anchor for a retrieval result.

    Retrograde summaries retain their own typed identity. Their
...
def get_total_chunks(db_conn) -> int:
    """
    Get the latest narrative id used as the temporal scale ceiling.

    Deleted legacy summary rows leave intentional gaps in narrative ids, so a
    row count is not a valid denominator for id-based chronology.

    Args:
        db_conn: Database connection

    Returns:
        Greatest current narrative chunk id
    """
    try:
        with db_conn.cursor() as cursor:
            cursor.execute("SELECT COALESCE(MAX(id), 0) FROM narrative_chunks")
            result = cursor.fetchone()
            if result and result[0]:
                return result[0]
    except Exception as e:
        logger.error(f"Error getting narrative temporal ceiling: {e}")
