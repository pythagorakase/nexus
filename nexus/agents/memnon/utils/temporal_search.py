...
def apply_temporal_boost(
    ...
    adjusted_score = (
        1.0 - temporal_boost_factor
    ) * base_score + temporal_boost_factor * temporal_relevance
...
def execute_time_aware_search(
    ...
    try:
        ...
        if temporal_classification == QUERY_NON_TEMPORAL:
            logger.debug(
                f"Query is non-temporal, using standard hybrid search: {query_text}"
            )
        ...
        logger.info(
            f"Performing time-aware search with classification: {temporal_classification}"
        )
