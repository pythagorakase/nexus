...
class QueryAnalyzer:
    ...
    def __init__(self, db_url: str, settings: Optional[Dict[str, Any]] = None) -> None:
        """
        Initialize the QueryAnalyzer

        Args:
            db_url: Explicit PostgreSQL database containing the slot cast.
            settings: Optional settings dictionary for customization
        """
        self.settings = settings or {}

        if not db_url or not make_url(db_url).database:
            raise ValueError("QueryAnalyzer requires an explicit database URL")
        with closing(psycopg2.connect(**url_connection_kwargs(db_url))) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT name FROM characters "
                    "UNION SELECT a.alias FROM character_aliases a "
                    "JOIN characters c ON c.id = a.character_id"
                )
                self.character_names = sorted(
                    {
                        row[0].strip()
        ...
        self.character_patterns = [
            rf"(?<!\w){re.escape(name)}(?!\w)" for name in self.character_names
        ] + [r"\bwho is\b", r"\bcharacter\b", r"\bperson\b"]

        self.location_patterns = self.settings.get(
            "location_patterns",
            [
                r"\bwhere\b",
                r"\blocation\b",
        ...
        self.event_patterns = self.settings.get(
            "event_patterns",
            [
                r"\bwhat happened\b",
                r"\bevent\b",
        ...
        self.relationship_patterns = self.settings.get(
        ...
        self.theme_patterns = self.settings.get(
            "theme_patterns",
            [r"\btheme\b", r"\bmotif\b", r"\bsymbolism\b", r"\bmeaning\b"],
        )
    ...
    def _compile_patterns(self) -> None:
        ...
        self.compiled_character_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.character_patterns
        ]
        self.compiled_location_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.location_patterns
        ]
        self.compiled_event_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.event_patterns
        ]
        self.compiled_relationship_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.relationship_patterns
        ]
        self.compiled_theme_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.theme_patterns
        ]
    ...
    def extract_entities(self, query_text: str) -> List[Dict[str, Any]]:
        ...
        for name in self.character_names:
            match = re.search(
                rf"(?<!\w){re.escape(name)}(?!\w)", query_text, re.IGNORECASE
            )
            if match:
                entities.append(
                    {
