...
from .continuous_temporal_search import (
...
class SearchManager:
    ...
    def query_text_search(
        ...
        try:
            ...
            for row in result:
                chunk_id, raw_text, season, episode, scene_number, score, highlights = (
                    row
                )
                text_results.append(
                    {
            ...
            # the same raw text-score ranking only when no narrative metadata
            # filter is active; silently ignoring such a filter would be wrong.
            summary_table_exists = False
            if _retrograde_summaries_allowed(filters):
                summary_table_exists = session.execute(
                    text("SELECT to_regclass('public.retrograde_summaries')")
                ).scalar()
                    ...
                    summary_result = session.execute(
                        text(
                            ...
                            SELECT
                                ...
                                created_at,
                                ts_rank(
                                    to_tsvector('english', summary_text),
                                ...
                                ts_headline(
                                    ...
                                    summary_text,
                                    websearch_to_tsquery('english', :query),
                                    'MaxFragments=3, MinWords=15, MaxWords=35'
                                ) AS highlights
                            FROM retrograde_summaries
                            WHERE to_tsvector('english', summary_text)
                                  @@ websearch_to_tsquery('english', :query)
                            ORDER BY score DESC
                            LIMIT :limit
                            """
