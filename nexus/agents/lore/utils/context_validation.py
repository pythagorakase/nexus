    ...
    if token_counts:
        ...
        if total_context_tokens > 0:
            ...
            if struct_pct < struct_range["min"]:
                errors.append(
                    f"Structured data below minimum: {struct_pct:.1f}% < {struct_range['min']}%"
                )
            elif struct_pct > struct_range["max"]:
                errors.append(
                    f"Structured data above maximum: {struct_pct:.1f}% > {struct_range['max']}%"
                )

            # Validate augmentation percentage
            if augment_pct < augment_range["min"]:
                errors.append(
                    f"Augmentation below minimum: {augment_pct:.1f}% < {augment_range['min']}%"
                )
            elif augment_pct > augment_range["max"]:
                errors.append(
                    f"Augmentation above maximum: {augment_pct:.1f}% > {augment_range['max']}%"
                )
...
def validate_token_allocation(
    ...
    if not (warm_min_tokens <= warm_tokens <= warm_max_tokens):
        errors.append(
            f"Warm slice allocation {warm_tokens:,} tokens outside bounds [{warm_min_tokens:,}, {warm_max_tokens:,}]"
        )
    ...
    if not (struct_min_tokens <= structured_tokens <= struct_max_tokens):
        errors.append(
            f"Structured data allocation {structured_tokens:,} tokens outside bounds [{struct_min_tokens:,}, {struct_max_tokens:,}]"
        )
    ...
    if not (augment_min_tokens <= augment_tokens <= augment_max_tokens):
        errors.append(
            f"Augmentation allocation {augment_tokens:,} tokens outside bounds [{augment_min_tokens:,}, {augment_max_tokens:,}]"
        )
