...
def assemble_chronological_context(
    warm_chunks: List[Dict], augment_chunks: List[Dict]
) -> List[Dict]:
    """
    ...
    logger.debug(
        f"Assembled {len(context_chunks)} chunks: "
...
def format_chunk_with_metadata(
    chunk_data: Dict, include_world_time: bool = True
) -> str:
    """
...
def merge_deduplicate_chunks(chunks1: List[Dict], chunks2: List[Dict]) -> List[Dict]:
    """
...
def filter_by_world_layer(
    chunks: List[Dict], allowed_layers: List[str] = None
) -> List[Dict]:
    """
    ...
    filtered = [c for c in chunks if c.get("world_layer", "primary") in allowed_layers]
...
def get_chunk_time_range(chunks: List[Dict]) -> Dict[str, Any]:
    ...
    primary_chunks = [
        c for c in chunks if c.get("world_layer") == "primary" and c.get("world_time")
    ]
