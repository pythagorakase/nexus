...
def resolve_seat_window(
    ...
    entry = APIModelEntry.model_validate(entries[0]).require_window_capabilities()
    apex = settings["apex"]
    ...
    assert entry.max_input_tokens is not None
    ...
    ceiling = min(
        ...
        entry.context_window - policy.max_output_tokens - outside,
    )
