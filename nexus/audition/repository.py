...
class AuditionRepository:
    ...
    def _define_tables(self) -> None:
        self.conditions = Table(
            ...
            Column(
                "provider",
                Enum("OpenAI", "Anthropic", "DeepSeek", "openrouter", name="provider_enum", schema="apex_audition"),
                nullable=False,
            ),
            Column(
                "model_name",
            ...
            Column("thinking_enabled", Boolean),
    ...
    def list_conditions(self, *, active_only: bool = False, visible_only: bool = False) -> List[ConditionSpec]:
        query = self.conditions.select()
        if active_only:
            query = query.where(self.conditions.c.is_active.is_(True))
        if visible_only:
            query = query.where(self.conditions.c.is_visible.is_(True))
        with self.engine.connect() as connection:
