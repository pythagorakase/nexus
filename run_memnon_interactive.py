...
def main():
    ...
    memnon = MEMNON(interface=interface, db_url=args.db_url, debug=args.debug)
            ...
            if command.get("action") == "status" or "test" in user_input.lower():
                ...
                if show_raw:
                    ...
                    query_results = memnon.query_memory(
                        query=user_input, query_type=None, filters={}, k=5
                    )
                    ...
                    print(
                        f"\033[1;36mSearch Plan:\033[0m {query_results['metadata']['search_plan']}"
                    )

                    # Print strategies used
                    strategies = query_results["metadata"]["search_stats"][
                        "strategies_executed"
                    ]
