...
- **validate-config** — fires when `nexus.toml` or any `.py` file is staged; makes model-roster edits safe to commit. Loads `nexus.toml` through the Pydantic models (invalid model selections and duplicate or unknown roster uses abort the commit here, not at the next boot), enforces the dev-dashboard ship-off invariant (`[orrery.dashboard] enabled` must be committed `false`; use `NEXUS_DEV_DASHBOARD=1` for local dashboard work), and runs `scripts/check_model_drift.py` for stale literal model ids lacking a `# pin: <reason>` comment.
...
- **Testing Defaults**: Do not hardcode model IDs in runtime defaults or live tests. Model IDs live in `[global.model.api_models.<provider>].models`; each entry's `uses` list names actual component fields (for example, `apex.model` or `wizard.default_model`). Change the entry's `id` once to upgrade all those configured uses. Read resolved component settings in runtime code; use registered model IDs for explicit overrides. The retired `roles` table and `@provider.role` aliases are rejected. Intentionally pinned regression fixtures should carry a `# pin: <reason>` comment for the drift checker.
