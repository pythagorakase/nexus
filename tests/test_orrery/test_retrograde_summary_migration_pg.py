...
def test_migration_078_reanchors_nested_maturation_job(
    disposable_retrograde_db: Any,
) -> None:
    """A job requested by legacy prose inherits the owning job's root chunk."""
    ...
    _drop_migration_targets(conn)
    seeded = _seed_legacy_runtime_summary(conn)
    nested_job_id = _seed_nested_maturation_job(
        conn,
        requesting_chunk_id=seeded["summary_id"],
        manifest_requesting_chunk_id=seeded["summary_id"],
    )
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT requesting_chunk_id, result_manifest "
            "FROM orrery_maturation_jobs WHERE id = %s",
            (nested_job_id,),
        )
        requesting_chunk_id, manifest = cur.fetchone()
        assert requesting_chunk_id == seeded["requesting_chunk_id"]
        assert manifest["requesting_chunk_id"] == seeded["requesting_chunk_id"]
...
def test_migration_078_rejects_nested_job_manifest_request_mismatch(
    disposable_retrograde_db: Any,
) -> None:
    """The migration never launders inconsistent column/manifest provenance."""
    ...
    nested_job_id = _seed_nested_maturation_job(
        conn,
        requesting_chunk_id=seeded["summary_id"],
        manifest_requesting_chunk_id=seeded["requesting_chunk_id"],
    )
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT requesting_chunk_id, result_manifest ->> 'requesting_chunk_id' "
            "FROM orrery_maturation_jobs WHERE id = %s",
            (nested_job_id,),
        )
        assert cur.fetchone() == (
            seeded["summary_id"],
            str(seeded["requesting_chunk_id"]),
        )
        cur.execute("SELECT to_regclass('public.retrograde_summaries')")
        assert cur.fetchone()[0] is None


def test_migration_078_reanchors_multi_hop_maturation_ancestry(
    disposable_retrograde_db: Any,
) -> None:
    """Every descendant job and summary collapses to the playable root."""
    ...
    _drop_migration_targets(conn)
    seeded = _seed_legacy_runtime_summary(conn)
    child_job_id = _seed_nested_maturation_job(
        conn,
        requesting_chunk_id=seeded["summary_id"],
        manifest_requesting_chunk_id=seeded["summary_id"],
    )
    child_summary_id = _seed_legacy_summary_for_job(
        conn,
        job_id=child_job_id,
        tick_chunk_id=seeded["requesting_chunk_id"],
    )
    grandchild_job_id = _seed_nested_maturation_job(
        conn,
        requesting_chunk_id=child_summary_id,
        manifest_requesting_chunk_id=child_summary_id,
    )
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            "result_manifest ->> 'requesting_chunk_id' "
            "FROM orrery_maturation_jobs WHERE id = ANY(%s) ORDER BY id",
            ([child_job_id, grandchild_job_id],),
        )
        assert cur.fetchall() == [
            (
                child_job_id,
        ...
        legacy_ids = [seeded["summary_id"], child_summary_id]
        cur.execute(
            "SELECT id, recorded_at_chunk_id FROM retrograde_summaries "
            "WHERE id = ANY(%s) ORDER BY id",
            (legacy_ids,),
        )
        assert cur.fetchall() == [
            (legacy_id, seeded["requesting_chunk_id"])
        ...
        cur.execute(
            ...
            "WHERE recorded_at_chunk_id = ANY(%s)",
            (legacy_ids,),
        )
        assert cur.fetchone()[0] == 0
        cur.execute(
            ...
            SELECT convalidated
            FROM pg_constraint
            WHERE conrelid = 'orrery_maturation_jobs'::regclass
              AND confrelid = 'narrative_chunks'::regclass
              AND contype = 'f'
            """
        )
...
def test_migration_078_rejects_json_null_manifest_without_laundering(
    disposable_retrograde_db: Any,
) -> None:
    """JSON null is malformed data, not an SQL NULL queue manifest."""
    ...
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE orrery_maturation_jobs SET result_manifest = 'null'::jsonb "
            "WHERE id = %s",
            (nested_job_id,),
        )
    conn.commit()

    with pytest.raises(RuntimeError, match="non-object manifest"):
        migration.run(conn)
    conn.rollback()

    with conn.cursor() as cur:
        cur.execute(
            "SELECT requesting_chunk_id, jsonb_typeof(result_manifest) "
            "FROM orrery_maturation_jobs WHERE id = %s",
            (nested_job_id,),
        )
        assert cur.fetchone() == (seeded["summary_id"], "null")
        cur.execute("SELECT to_regclass('public.retrograde_summaries')")
        assert cur.fetchone()[0] is None


def test_migration_078_rolls_back_after_late_postcondition_failure(
    disposable_retrograde_db: Any,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A failure after every mutation restores all legacy artifacts."""
    ...
    validate_postconditions = migration._validate_postconditions

    def _fail_after_postconditions(*args: Any, **kwargs: Any) -> None:
        validate_postconditions(*args, **kwargs)
        raise RuntimeError("forced late migration failure")

    monkeypatch.setattr(
        migration, "_validate_postconditions", _fail_after_postconditions
    )

    with pytest.raises(RuntimeError, match="forced late migration failure"):
        migration.run(conn)
    conn.rollback()
    ...
    with conn.cursor() as cur:
        ...
        assert cur.fetchone() == (
            seeded["summary_id"],
            str(seeded["summary_id"]),
        )
        cur.execute(
            "SELECT payload ? 'retrograde_summary_chunk_id' "
        ...
        assert cur.fetchone()[0] == 1
        cur.execute(
            "SELECT count(*) FROM chunk_embeddings_0004d WHERE chunk_id = %s",
            (seeded["summary_id"],),
        )
        assert cur.fetchone()[0] == 1
        cur.execute("SELECT to_regclass('public.retrograde_summaries')")
...
def test_migration_078_accepts_complete_legacy_embedding_catalog(
    disposable_retrograde_db: Any,
) -> None:
    """Historical varchar/nullable DDL is safe when every timestamp exists."""
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT embedding::text, created_at FROM chunk_embeddings_0004d "
            "WHERE chunk_id = %s AND model = 'migration-test'",
            (seeded["summary_id"],),
        )
        source_embedding, source_created_at = cur.fetchone()
        cur.execute(
            "SELECT conname FROM pg_constraint "
            "WHERE conrelid = 'chunk_embeddings_0004d'::regclass "
            "AND confrelid = 'narrative_chunks'::regclass AND contype = 'f'"
        )
        (source_fk,) = cur.fetchone()
        cur.execute(
            sql.SQL("ALTER TABLE chunk_embeddings_0004d DROP CONSTRAINT {}").format(
                sql.Identifier(source_fk)
            )
        )
        cur.execute(
            "ALTER TABLE chunk_embeddings_0004d "
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT summary_id, model, embedding::text, created_at "
            "FROM retrograde_summary_embeddings_0004d"
        )
        assert cur.fetchall() == [
            (
                seeded["summary_id"],
        ...
        cur.execute(
            ...
            FROM pg_attribute
            WHERE attrelid = 'retrograde_summary_embeddings_0004d'::regclass
              AND attname IN ('summary_id', 'model', 'embedding', 'created_at')
            ORDER BY attname
            """
        )
        assert cur.fetchall() == [
...
def test_migration_078_rejects_null_legacy_embedding_timestamp(
    disposable_retrograde_db: Any,
) -> None:
    """Nullable historical DDL does not authorize lossy NULL timestamps."""
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            "UPDATE chunk_embeddings_0004d SET created_at = NULL "
            "WHERE chunk_id = %s",
            (seeded["summary_id"],),
        )
    conn.commit()

    with pytest.raises(RuntimeError, match="NULL created_at"):
        migration.run(conn)
    conn.rollback()


def test_migration_078_repairs_empty_slot_lifecycle_columns(
    disposable_retrograde_db: Any,
) -> None:
    """Setup-only slots retain data while their empty chunk table is repaired."""
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            "DROP COLUMN choice_object, DROP COLUMN choice_text, "
            "DROP COLUMN state, DROP COLUMN embedding_generated_at"
        )
    conn.commit()
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE c.table_schema = 'public'
              AND c.table_name = 'narrative_chunks'
              AND c.column_name IN ('state', 'embedding_generated_at')
            ORDER BY c.column_name
            """
        )
        assert cur.fetchall() == [
            ("embedding_generated_at", "timestamp with time zone", None),
...
@pytest.mark.parametrize(
    ...
    [
        ("narrative_chunks", "choice_text"),
        ("chunk_metadata", "scene"),
    ],
)
def test_migration_078_requires_full_copy_schema_for_legacy_rows(
    disposable_retrograde_db: Any,
    table_name: str,
    column_name: str,
) -> None:
    """A nonempty legacy set cannot bypass any heavy-copy column check."""
    ...
    _drop_migration_targets(conn)
    _seed_legacy_runtime_summary(conn)
    with conn.cursor() as cur:
        cur.execute(
    ...
    with pytest.raises(RuntimeError, match=column_name):
        migration.run(conn)
    conn.rollback()
...
def test_migration_078_refuses_lifecycle_repair_on_populated_slot(
    disposable_retrograde_db: Any,
) -> None:
    """Missing lifecycle columns on a populated table remain ambiguous."""
    ...
    with pytest.raises(RuntimeError, match="cannot repair populated"):
        migration.run(conn)
    conn.rollback()
...
def _seed_nested_maturation_job(
    conn: Any,
    *,
    requesting_chunk_id: int,
    manifest_requesting_chunk_id: int,
) -> int:
    """Create a job whose request points at prose emitted by an earlier job."""

    with conn.cursor() as cur:
        unique_name = f"Nested Migration Courier {uuid.uuid4().hex[:8]}"
        cur.execute(
            """
            INSERT INTO characters (
                name, summary, background, current_activity, extra_data
            ...
            (
                unique_name,
                "Disposable nested migration-test character.",
                "Created only inside a disposable database.",
                "Testing migration 078 ancestry repair.",
            ),
        )
        character_id, entity_id = map(int, cur.fetchone())
        manifest = {
            "schema_version": "orrery_retrograde_maturation_manifest.v0",
            "persisted": False,
            "requesting_chunk_id": manifest_requesting_chunk_id,
        }
        cur.execute(
            """
            INSERT INTO orrery_maturation_jobs (
...
def _seed_legacy_summary_for_job(
    conn: Any,
    *,
    job_id: int,
    tick_chunk_id: int,
) -> int:
    """Create a second canonical legacy summary owned by ``job_id``."""

    event_ref = f"maturation_job_{job_id}_event_ancestry"
    summary_text = f"Nested migration provenance for job {job_id}."
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO narrative_chunks (
        ...
        _insert_metadata(cur, summary_id, scene=3, world_layer="retrograde")
        cur.execute("SELECT type FROM event_types ORDER BY type LIMIT 1")
        event_type = cur.fetchone()[0]
        cur.execute(
            """
