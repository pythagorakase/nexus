...
class IRDatabase:
    ...
    def add_query(
        self, text: str, category: str = None, name: str = None
    ) -> Optional[int]:
        ...
        try:
            ...
            for query_data in query_results:
                ...
                for rank, result in enumerate(results, 1):
                    ...
                    logger.info(
                        f"TEXT SCORE DEBUG - doc_id={doc_id}, text_score={text_score}, type={type(text_score).__name__}"
                    )
                    ...
                    logger.info(
                        f"SQL PARAM DEBUG - text_score_param={text_score_param}, type={type(text_score_param).__name__}"
                    )
                    ...
                    logger.info(
                        f"SQL PARAM LIST - {[type(p).__name__ for p in params]}"
                    )
        ...
        try:
            ...
            for query in queries:
                ...
                if len(all_rows) > 0:
                    ...
                    print(
                        f"  vector_score type: {type(first_row.get('vector_score')).__name__}"
                    )
                    print(f"  text_score: {first_row.get('text_score')}")
                    print(
                        f"  text_score type: {type(first_row.get('text_score')).__name__}"
                    )
                    ...
                    if raw_row:
                        ...
                        print(
                            f"  RAW SQL vector_score type: {type(raw_row['vector_score']).__name__}"
                        )
                        print(
                            f"  RAW SQL text_score type: {type(raw_row['text_score']).__name__}"
                        )
                    ...
                    for col in columns:
                        if col["name"] in ["vector_score", "text_score"]:
                            print(
                                f"  Column {col['name']} type in SQLite schema: {col['type']}"
                            )
                ...
                for r in all_rows:
                    ...
                    if vector_score is not None:
                        try:
                            ...
                            if vector_score < 0 or vector_score > 1:
                                print(
                                    f"WARNING: Vector score {vector_score} outside expected range [0,1]"
                                )
                        except (ValueError, TypeError):
                            print(
                                f"WARNING: Could not convert vector_score '{vector_score}' (type: {type(vector_score).__name__}) to float"
                            )
                            vector_score = 0.0
                    else:
                        vector_score = 0.0
                    ...
                    if text_score is not None:
                        try:
                            ...
                            if text_score < 0 or text_score > 1:
                                print(
                                    f"WARNING: Text score {text_score} outside expected range [0,1]"
                                )
                        except (ValueError, TypeError):
                            print(
                                f"WARNING: Could not convert text_score '{text_score}' (type: {type(text_score).__name__}) to float"
                            )
                            text_score = 0.0
                    else:
                        text_score = 0.0
                    ...
                    print(
                        f"  Processed text_score for doc {r.get('doc_id')}: {text_score}"
                    )
                    ...
                    try:
                        ...
                        if raw_result:
                            ...
                            print(
                                f"  DIRECT DB text_score for doc {r.get('doc_id')}: {raw_text_score} (type: {type(raw_text_score).__name__})"
                            )
    ...
    def get_run_metrics(self, run_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if run:
                ...
                metadata = {"timestamp": run["timestamp"], "settings": settings}
            else:
    ...
    def link_run_pair(
        self, control_run_id: int, experiment_run_id: int, description: str
    ) -> bool:
        ...
        try:
            ...
            self.conn.commit()
            logger.info(
                f"Linked runs {control_run_id} and {experiment_run_id} as a pair"
            )
            return True
        except sqlite3.Error as e:
