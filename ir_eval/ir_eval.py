...
# Constants
DEFAULT_GOLDEN_QUERIES_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "golden_queries.json"
)
...
def extract_memnon_settings(settings_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        "Agent Settings" in settings_data
        and "MEMNON" in settings_data["Agent Settings"]
    ):
        return settings_data["Agent Settings"]["MEMNON"]
    return {}


def create_temp_settings_file(
    settings_data: Dict[str, Any], memnon_override: Dict[str, Any]
) -> str:
    """
    ...
    if (
        "Agent Settings" in settings_copy
        and "MEMNON" in settings_copy["Agent Settings"]
    ):
        # Apply selective overrides
        for key, value in memnon_override.items():
            if key in settings_copy["Agent Settings"]["MEMNON"]:
                if isinstance(value, dict) and isinstance(
                    settings_copy["Agent Settings"]["MEMNON"][key], dict
                ):
                    ...
                    if key == "models":
                        for model_name, model_config in value.items():
                            if (
                                model_name
                                in settings_copy["Agent Settings"]["MEMNON"]["models"]
                            ):
                                # Preserve local_path from original settings
                                local_path = settings_copy["Agent Settings"]["MEMNON"][
                                    "models"
                                ][model_name].get("local_path")
                                if local_path:
                                    if (
                                        model_name
                                        not in settings_copy["Agent Settings"][
                                            "MEMNON"
                                        ]["models"]
                                    ):
                                        settings_copy["Agent Settings"]["MEMNON"][
                                            "models"
                                        ][model_name] = {}
                                    settings_copy["Agent Settings"]["MEMNON"]["models"][
                                        model_name
                                    ].update(model_config)
                                    settings_copy["Agent Settings"]["MEMNON"]["models"][
                                        model_name
                    ...
                    # Special handling for retrieval settings which has nested dictionaries
                        ...
                        for subkey, subvalue in value.items():
                            if (
                                subkey
                                in settings_copy["Agent Settings"]["MEMNON"][
                                    "retrieval"
                                ]
                                and isinstance(subvalue, dict)
                                and isinstance(
                                    settings_copy["Agent Settings"]["MEMNON"][
                                        "retrieval"
                                    ][subkey],
                                ...
                                settings_copy["Agent Settings"]["MEMNON"]["retrieval"][
                                    subkey
                                ].update(subvalue)
                            else:
        ...
        logger.info(
            f"Created experimental settings with hybrid_search config: "
        ...
        logger.info(
            f"Cross encoder config in experiment: {json.dumps(cross_encoder_config, indent=2)}"
        )
...
def get_query_data_by_category(
...
def run_golden_queries(
    ...
    try:
        ...
        logger.info(
            f"Attempting to import golden_queries_module from: {local_scripts_dir}"
        )
        ...
        if module_path != expected_path:
            logger.warning(
                f"WARNING: Imported golden_queries_module from unexpected location: {module_path}"
            )
        ...
        logger.info(
            f"Successfully imported golden_queries_module from {golden_queries_module.__file__}"
        )
        ...
        try:
            ...
            if (
                results_data
                and "query_results" in results_data
                and results_data["query_results"]
            ):
            ...
            if not run_description:
                run_description = (
                    f"Generated by ir_eval_pg.py with {config_type} configuration"
                )
        ...
        logger.error(
            "This indicates the refactoring is incomplete - you need to create scripts/golden_queries_module.py"
        )
        return None
...
def judge_all_unjudged_results(
    ...
    for run_id in run_ids:
        ...
        for query_data in query_results:
            ...
            unjudged_count = sum(
                1 for r in results if str(r.get("id")) not in judged_docs
            )
            total_unjudged_count += unjudged_count
            total_results_count += len(results)

    logger.info(
        f"Found {total_unjudged_count} total unjudged documents out of {total_results_count}"
    )
    ...
    for run_id in run_ids:
        ...
        logger.info(
            f"Loaded {len(query_results)} queries from database for run_id {run_id}"
        )

        # Get run metadata
        run_data = db.get_run_metadata(run_id)
        run_name = (
            run_data.get("name", f"Run {run_id}") if run_data else f"Run {run_id}"
        )
        ...
        for query_data in query_results:
            ...
            query_info = None
            if (
                category in golden_queries_data
                and name in golden_queries_data[category]
            ):
            ...
            print(
                f"TOTAL REMAINING: {total_unjudged_count} documents left to judge across all queries"
            )
            print("★" * 100)
            ...
            while i < len(results):
                ...
                print("\n" + "=" * 80)
                print(
                    f"Document {i+1}/{len(results)} (Chunk ID: {chunk_id}, {score_display})"
                )
                ...
                while True:
                    relevance_input = (
                        input("Relevance (0-3), S to skip, U to undo, or Q to quit: ")
                        .strip()
                        .upper()
                    )
                    ...
                    if relevance_input == "U" and last_judgment:
                        ...
                        if (
                            undo_query == query_text
                            and str(undo_chunk_id) in judged_docs
                        ):
                    ...
                    if relevance_input == "S":
                        ...
                        print(
                            f"PROGRESS: {len(judged_docs)}/{len(results)} judged | {remaining_in_current_query} remaining for this query"
                        )
                        print(
                            f"TOTAL REMAINING: {total_unjudged_count} documents left to judge across all queries"
                        )
                    ...
                    try:
                        ...
                        if 0 <= relevance <= 3:
                            # Add judgment
                            qrels.add_judgment(
                                query_text, chunk_id, relevance, category, text
                            )
                            ...
                            print(
                                "Please enter a number between 0 and 3, or Q to quit, U to undo, S to skip"
                            )
                    except ValueError:
                        print(
                            "Please enter a number between 0 and 3, or Q to quit, U to undo, S to skip"
...
def evaluate_run(
    run_id: int, qrels: PGQRELSManager, db: Optional[IRDatabasePG] = None
) -> Dict[str, Any]:
    """
...
def compare_runs(
    run_ids: List[int], run_names: List[str], db: Optional[IRDatabasePG] = None
) -> Dict[str, Any]:
    """
    ...
    for run_id in run_ids:
        ...
        if query_results:
            ...
            for query_data in query_results:
                ...
                if (
                    "metadata" in query_data
                    and "reranking_time" in query_data["metadata"]
                ):
            ...
            if query_count > 0:
                ...
                if has_reranking_stats:
                    runtime_stats["avg_reranking_time"] = (
                        total_reranking_time / query_count
                    )
    ...
    comparison = {
        ...
        "runs": [
            ...
            for name, results in zip(run_names, run_results)
        ],
        "comparison": {"overall": {}, "by_category": {}},
        "best_run": {},
    }
    ...
    for category in all_categories:
        ...
        for metric in ["p@5", "p@10", "mrr", "bpref"]:
            ...
            for run in run_results:
                if (
                    run
                    and "aggregated" in run
                    and "by_category" in run["aggregated"]
                    and category in run["aggregated"]["by_category"]
                ):
                    values.append(
                        run["aggregated"]["by_category"][category].get(metric, 0)
                    )
                else:
                    values.append(0)
    ...
    if has_runtime_stats:
        ...
        for run in run_results:
            if (
                run
                and "runtime_stats" in run
                and "avg_reranking_time" in run["runtime_stats"]
            ):
            ...
            for run in run_results:
                if (
                    ...
                    reranking_times.append(
                        run["runtime_stats"].get("avg_reranking_time", 0)
                    )
                else:
...
def print_comparison_table(comparison: Dict[str, Any]) -> None:
    ...
    if has_timing_data:
        ...
        if (
            len(avg_times) == 2
            and avg_times[0] is not None
            and avg_times[1] is not None
        ):
        ...
        for run in comparison["runs"]:
            if (
                run["results"]
                and "runtime_stats" in run["results"]
                and "avg_reranking_time" in run["results"]["runtime_stats"]
            ):
        ...
        if has_reranking_time:
            ...
            for run in comparison["runs"]:
                if (
                    ...
                    rerank_time = run["results"]["runtime_stats"].get(
            ...
            if (
                len(rerank_times) == 2
                and rerank_times[0] is not None
                and rerank_times[1] is not None
            ):
    ...
    for run in comparison["runs"]:
        if (
            run["results"]
            and "aggregated" in run["results"]
            and "overall" in run["results"]["aggregated"]
        ):
...
class IREvalPGCLI:
    ...
    def __init__(self):
        ...
        self.latest_run_ids = self.db.get_latest_run_ids(
            [CONTROL_CONFIG_NAME, EXPERIMENT_CONFIG_NAME]
        )
        ...
        if "settings" in self.golden_queries:
            ...
            if "models" in self.golden_queries["settings"]:
                if not self.experimental_settings:
                    self.experimental_settings = {}
                self.experimental_settings["models"] = self.golden_queries["settings"][
                    "models"
                ]
    ...
    def run_all_queries(self):
        ...
        if self.experimental_settings:
            ...
            temp_settings_path = create_temp_settings_file(
                self.settings, self.experimental_settings
            )
            ...
            if exp_run_id:
                ...
                self.db.link_run_pair(
                    control_run_id, exp_run_id, experiment_description
                )
                print(
                    f"Control run {control_run_id} and Experimental run {exp_run_id} linked as a pair."
                )
    ...
    def run_category_queries(self):
        ...
        while True:
            try:
                choice = int(
                    input(
                        "\nSelect category (1-{0}): ".format(
                            len(queries_by_category) + 1
                        )
                    )
                )
        ...
        if self.experimental_settings:
            ...
            if exp_run_id:
                ...
                self.db.link_run_pair(
                    control_run_id,
                    exp_run_id,
                    f"{experiment_description} - {category} category",
                )
                print(
                    f"Control run {control_run_id} and Experimental run {exp_run_id} linked as a pair."
                )
    ...
    def judge_results(self):
        ...
        judgments_added = judge_all_unjudged_results(
            run_ids, self.qrels, self.golden_queries, self.db
        )
    ...
    def compare_results(self):
        ...
        for i, pair in enumerate(pairs, 1):
            ...
            experiment_name = pair.get(
                "experiment_name", f"Experiment {pair['experiment_run_id']}"
            )
        ...
        if total_unjudged > 0:
            ...
            for run_id, count in unjudged_counts.items():
                if count > 0:
                    ...
                    run_name = (
                        run_info.get("name", f"Run {run_id}")
                        if run_info
                        else f"Run {run_id}"
                    )
            ...
            if judge_now == "y":
                ...
                added = judge_all_unjudged_results(
                    run_ids, self.qrels, self.golden_queries, self.db
                )
    ...
    def view_configurations(self):
        ...
        if self.control_settings:
            if "retrieval" in self.control_settings:
                ...
                print(
                    f"  Relevance threshold: {retrieval.get('relevance_threshold', 'N/A')}"
                )
                print(
                    f"  Entity boost factor: {retrieval.get('entity_boost_factor', 'N/A')}"
                )
                ...
                if "hybrid_search" in retrieval:
                    ...
                    print(
                        f"  Query-specific temporal factors: {hybrid.get('use_query_type_temporal_factors', False)}"
                    )
            ...
            if "models" in self.control_settings:
                ...
                for model, config in self.control_settings["models"].items():
                    ...
                    if active:
                        print(
                            f"  {model}: dimensions={config.get('dimensions', 'N/A')}, weight={config.get('weight', 'N/A')}"
                        )
        else:
    ...
    def display_current_parameters(self):
        ...
        try:
            ...
            try:
                ...
                settings_compare.show_settings_management_menu(
                    self.settings_path, self.golden_queries_path
                )

            except ImportError:
                logger.error(
                    "Could not import settings_compare module, using basic display instead"
                )
    ...
    def _display_current_parameters_basic(self):
        ...
        memnon_settings = {}
        if (
            "Agent Settings" in settings_content
            and "MEMNON" in settings_content["Agent Settings"]
        ):
        ...
        # Print helpful note
        print(
            "The above JSON blocks can be copied and pasted directly for use in discussions about parameter tuning."
        )
    ...
    def delete_runs(self):
        ...
        print("=" * 80)
        print(
            "WARNING: Deleting runs will permanently remove the run data and all associated results!"
        )
        ...
        for i, run in enumerate(runs, 1):
            ...
            print(
                f"{i}. {run.get('name', 'Unnamed')} (ID: {run.get('id')}) - {run_time_display}"
            )
        ...
        try:
            ...
            if 1 <= choice <= len(runs):
                ...
                # Confirm deletion
                confirm = input(
                    f"\nAre you sure you want to delete run {run_name} (ID: {run_id})? (y/n): "
                ).lower()
                if confirm == "y":
                    # Attempt to delete run
                    if self._delete_run(run_id):
                        print(
                            f"Run {run_name} (ID: {run_id}) has been deleted successfully."
                        )
                    else:
    ...
    def _delete_run(self, run_id: int) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM ir_eval.run_pair_links WHERE control_run_id = %s OR experiment_run_id = %s",
                (run_id, run_id),
            )
    ...
    def ai_judge_results(self):
        ...
        print("=" * 80)
        print(
            "This will use OpenAI API to automatically judge unjudged search results."
        )
        ...
        try:
            ...
            dry_run = (
                input("Dry run (don't save judgments)? (y/n, default: n): ")
                .strip()
                .lower()
                == "y"
            )
            debug = (
                input("Debug mode (verbose output)? (y/n, default: n): ")
                .strip()
                .lower()
                == "y"
            )
            ...
            # Initialize AI Judge
            ai_judge = AIJudge(
                model=model, temperature=temperature, dry_run=dry_run, debug=debug
            )
            ...
            if 1 <= choice <= len(runs):
                ...
                judgments = judge_run(
                    run_id=run_id, ai_judge=ai_judge, db=self.db, qrels=self.qrels
                )
    ...
    def _judge_multiple_runs(
        self, available_runs: List[Dict[str, Any]], ai_judge: AIJudge
    ) -> None:
        """
        ...
        # Handle comma-separated list
            ...
            try:
                ...
                valid_indices = [
                    idx for idx in selected_indices if 1 <= idx <= len(available_runs)
                ]
        ...
        if run_ids:
            run_names = ", ".join(
                [
                    f"{run.get('name', 'Unnamed')} (ID: {run.get('id')})"
            ...
            if confirm == "y":
                ...
                for run_id in run_ids:
                    ...
                    run_info = next(
                        (run for run in available_runs if run.get("id") == run_id), None
                    )
                    run_name = (
                        run_info.get("name", f"Run {run_id}")
                        if run_info
        ...
        if run_ids_to_delete:
            run_names = ", ".join(
                [
                    f"{run.get('name', 'Unnamed')} (ID: {run.get('id')})"
                    for run in available_runs
                    if run.get("id") in run_ids_to_delete
                ]
            )
            ...
            if confirm == "y":
                ...
                if (
                    is_deleting_all
                    and success_count == len(run_ids_to_delete)
                    and success_count > 0
                ):
                    try:
                        cursor = self.db.conn.cursor()
