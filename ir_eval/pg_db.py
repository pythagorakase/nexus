...
class IRDatabasePG:
    ...
    def __init__(
        ...
        *,
        ...
        """
        self.connection_params = connection_kwargs(
            dbname, user=user, host=host, port=port
        )
        self.conn = None
        self._connect()
        self._ensure_schema()
    ...
    def _ensure_schema(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'ir_eval'"
            )
            if not cursor.fetchone():
                # Schema doesn't exist, create from schema file
                schema_path = os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "pg_schema.sql"
                )
    ...
    def get_query_id(self, query_text: str) -> Optional[int]:
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "SELECT id FROM ir_eval.queries WHERE text = %s", (query_text,)
            )
            ...
            if existing_id:
                ...
                if category or name:
                    ...
                    if updates:
                        ...
                        cursor.execute(
                            f"UPDATE ir_eval.queries SET {', '.join(updates)} WHERE id = %s",
                            params,
                        )
                        self.conn.commit()
        ...
        try:
            ...
            for query_data in query_results:
                ...
                for rank, result in enumerate(results, 1):
                    ...
                    if not chunk_id:
                        logger.warning(
                            f"Missing chunk_id in result for query: {query_text}"
                        )
                        continue
    ...
    def get_runs(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT * FROM ir_eval.runs ORDER BY timestamp DESC LIMIT %s", (limit,)
            )
    ...
    def get_run_results(self, run_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for query in queries:
                ...
                if (
                    first_result
                    and "elapsed_time" in first_result
                    and first_result["elapsed_time"] is not None
                ):
                    results_with_metadata["elapsed_time"] = float(
                        first_result["elapsed_time"]
                    )

                # Get metadata if available from first result
                if (
                    first_result
                    and "metadata" in first_result
                    and first_result["metadata"] is not None
                ):
                    try:
                        if isinstance(first_result["metadata"], str):
                            results_with_metadata["metadata"] = json.loads(
                        ...
                        logger.warning(
                            f"Could not parse metadata JSON for query: {query['text']}"
                        )
            ...
            for run in runs:
                ...
                for query_data in results:
                    ...
                    if run_id in [1, 2]:  # Only for first couple of runs
                        ...
                        logger.info(
                            f"Run {run_id}, Query '{query_text}' - first doc IDs: {doc_ids}"
                        )
                ...
                if has_unjudged:
                    logger.info(
                        f"Run {run_id} ({run.get('name', '')}) has unjudged results: {unjudged_counts}"
                    )
                    runs_with_unjudged.append(run)
                else:
                    logger.info(
                        f"Run {run_id} ({run.get('name', '')}) has NO unjudged results"
                    )
