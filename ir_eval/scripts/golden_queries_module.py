    ...
    if "retrieval" in memnon_settings:
        ...
        summary["retrieval"] = {
            ...
            "user_character_focus_boost": {
                "enabled": retrieval.get("user_character_focus_boost", {}).get(
                    "enabled", False
                )
            },
        }
        ...
        if "hybrid_search" in retrieval:
            summary["hybrid_search"] = {
                ...
                "vector_weight_default": retrieval["hybrid_search"].get(
                    "vector_weight_default", 0
                ),
        ...
        if "cross_encoder_reranking" in retrieval:
            summary["cross_encoder_reranking"] = {
                ...
                "model_path": retrieval["cross_encoder_reranking"].get(
                    "model_path", ""
                ),
                "blend_weight": retrieval["cross_encoder_reranking"].get(
                    "blend_weight", 0
                ),
                "top_k": retrieval["cross_encoder_reranking"].get("top_k", 0),
                "batch_size": retrieval["cross_encoder_reranking"].get("batch_size", 0),
                "use_sliding_window": retrieval["cross_encoder_reranking"].get(
                    "use_sliding_window", False
                ),
                "window_size": retrieval["cross_encoder_reranking"].get(
                    "window_size", 0
                ),
                "window_overlap": retrieval["cross_encoder_reranking"].get(
                    "window_overlap", 0
                ),
                "use_query_type_weights": retrieval["cross_encoder_reranking"].get(
                    "use_query_type_weights", False
                ),
                "weights_by_query_type": retrieval["cross_encoder_reranking"].get(
...
def format_settings_summary_text(settings: Dict[str, Any]) -> str:
    ...
    if "models" in settings:
        active_models = [
            f"{name} (weight={config.get('weight', 0.0)})"
    ...
    if "cross_encoder_reranking" in settings:
        ...
        if ce.get("enabled", False):
            ...
            window_status = (
                "ENABLED" if ce.get("use_sliding_window", False) else "DISABLED"
            )
            ...
            query_weights_status = (
                "ENABLED" if ce.get("use_query_type_weights", False) else "DISABLED"
            )
            lines.append(f"  Query-specific weights: {query_weights_status}")
            if (
                ce.get("use_query_type_weights", False)
                and "weights_by_query_type" in ce
            ):
    ...
    if "retrieval" in settings:
        ...
        structured_status = (
            "ENABLED" if retrieval.get("structured_data_enabled", True) else "DISABLED"
        )
...
def run_queries(
    ...
    try:
        ...
        try:
            ...
            ir_eval_dir = os.path.join(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            )
            alternative_path = os.path.join(ir_eval_dir, "golden_queries.json")
            logger.info(
                f"Original path not found, trying alternative: {alternative_path}"
            )
    ...
    try:
        ...
        if not query_rows:
            ...
            if total_count > 0:
                logger.error(
                    f"Database has {total_count} queries, but none were returned for this request"
                )
                if category:
                    logger.error(
                        f"Category filter '{category}' may be invalid or no queries match this category"
                    )
                else:
                    logger.error(
                        "Failed to retrieve ANY queries despite no category filter"
                    )
            else:
                logger.error("Database has NO queries at all")

            raise ValueError(
                f"No queries found in database (total queries: {total_count})"
            )
        ...
        for row in query_rows:
            ...
            if len(all_queries) <= 3:
                logger.info(
                    f"Sample query {len(all_queries)}: id={query_id}, text='{query_text[:30]}...', category={query_category}"
                )
    ...
    if k is None:
        ...
        # Next try from experimental settings in golden_queries.json
            ...
            "settings" in golden_queries_data
            and "query" in golden_queries_data["settings"]
            and "chunks_per_query" in golden_queries_data["settings"]["query"]
        ):
    ...
    for i, query_info in enumerate(all_queries):
        ...
        try:
            ...
            query_result = memnon.query_memory(
                query=query_text, query_type=query_type, k=k
            )

            elapsed_time = time.time() - start_time
            logger.info(
                f"Query completed in {elapsed_time:.2f}s with {len(query_result['results'])} results"
            )
            ...
            for i, res in enumerate(query_result["results"][:3]):
                ...
                logger.info(
                    f"Result {i}: id={res.get('id', 'unknown')}, "
            ...
            for res in query_result["results"]:
                ...
                if "vector_score" not in processed or processed["vector_score"] is None:
                    ...
                    logger.info(
                        f"Setting missing vector_score to 0.0 for chunk {processed.get('id', 'unknown')}"
                    )
                else:
                    try:
            ...
            if "rerank_time" in metadata:
                logger.info(
                    f"Cross-encoder reranking took {metadata['rerank_time']:.3f} seconds for query '{query_text}'"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hybrid", action="store_true", default=None, help="Force enable hybrid search"
    )
    parser.add_argument(
        "--no-hybrid",
        dest="hybrid",
