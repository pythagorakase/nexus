    ...
    try:
        ...
        try:
            ...
            if not run_description:
                run_description = (
                    f"Generated by query_runner.py with {config_type} configuration"
                )
...
def run_queries_for_specific_ids(
    ...
    try:
        ...
        cursor.execute(
            f"SELECT id, text, category FROM queries WHERE id IN ({placeholders})",
            query_ids,
        )
        ...
        for query in queries:
            ...
            try:
                ...
                results = memnon.search(query=query_text, k=k, include_chunks=True)
                ...
                logger.info(
                    f"Got {len(formatted_results)} results for query ID {query_id}"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run golden queries and save results to database"
    )
    parser.add_argument("--settings", default=None, help="Path to settings.json")
    parser.add_argument(
        "--golden-queries", default=None, help="Path to golden_queries.json"
    )
    ...
    if args.query_ids:
        ...
        if run_id:
            print(
                f"Successfully ran golden queries and saved results with run ID: {run_id}"
            )
        else:
