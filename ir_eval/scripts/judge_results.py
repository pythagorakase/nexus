...
def main():
    ...
    parser.add_argument(
        "--results", required=True, help="Path to the results JSON file"
    )
    parser.add_argument(
        "--qrels",
        default="qrels.json",
    ...
    parser.add_argument(
        "--skip-judged", action="store_true", help="Skip already judged documents"
    )
    ...
    judge_results_interactive(
        args.results, args.qrels, args.golden_queries, args.skip_judged
    )
