...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("auto_judge.log"), logging.StreamHandler()],
)
    ...
    def judge_chunk(
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if score_match:
                        ...
                        justification_match = re.search(
                            r'"justification"\s*:\s*"([^"]*)"', content
                        )
                        justification = (
                            justification_match.group(1)
                            if justification_match
                            else "No justification found"
                        )
                        return score, justification
                except:
                    pass
                ...
                logger.warning(
                    f"Failed to parse relevance score for chunk {chunk_id}, defaulting to 0"
                )
...
def get_query_examples(db: IRDatabasePG, query_id: int) -> List[str]:
    ...
    try:
        ...
        cursor.execute(
            "SELECT examples FROM ir_eval.queries WHERE id = %s", (query_id,)
        )
...
def get_unjudged_results(
    db: IRDatabasePG, qrels: PGQRELSManager, run_id: int
) -> List[Dict[str, Any]]:
    """
...
def judge_run(
    ...
    for i, result in enumerate(unjudged_results):
        ...
        logger.info(
            f"Processing result {i+1}/{len(unjudged_results)} - Chunk {chunk_id}, Query: {query_text[:50]}..."
        )
        ...
        logger.info(
            f"Judgment for chunk {chunk_id}: Score = {relevance}, Reasoning = {reasoning[:100]}..."
        )
...
def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be judged but don't update database",
    )
    parser.add_argument(
    ...
    if not args.disable_abort:
        setup_abort_handler(
            "Abort requested! Finishing current judgment and stopping..."
        )
    ...
    for run_id in run_ids:
        judgments = judge_run(run_id=run_id, ai_judge=ai_judge, db=db, qrels=qrels)
    ...
    logger.info(
        f"Average time per judgment: {elapsed / max(1, total_judgments):.2f} seconds"
    )
    logger.info("=" * 60)
