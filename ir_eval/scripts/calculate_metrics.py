        ...
        results.append(
            {
                ...
                "text_score": (
                    float(row["text_score"]) if row["text_score"] is not None else 0.0
                ),
                "text": row["text"],
                "source": row["source"],
            }
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate metrics for all queries in a run"
    )
    parser.add_argument(
        "--run-id", type=int, required=True, help="Run ID to calculate metrics for"
    )
    ...
    if not db_path:
        db_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "ir_eval.db"
        )
    ...
    logger.info(
        f"Found {len(missing_query_ids)} queries missing metrics for run ID {args.run_id}"
    )
    ...
    logger.info(
        f"Successfully calculated metrics for {success_count}/{len(missing_query_ids)} queries"
    )
