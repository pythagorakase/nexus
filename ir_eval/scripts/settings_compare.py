...
def compare_settings(settings_path: str, golden_queries_path: str) -> Dict[str, Any]:
    ...
    for key in hybrid_settings:
        ...
        if control_value != exp_value:
            hybrid_diffs[key] = {"control": control_value, "experimental": exp_value}
    ...
    query_types = set(
        list(control_weights_by_query_type.keys())
        + list(exp_weights_by_query_type.keys())
    )
    ...
    for query_type in query_types:
        ...
        if control_vector != exp_vector or control_text != exp_text:
            weights_by_query_type_diffs[query_type] = {
                "control": {"vector": control_vector, "text": control_text},
                "experimental": {"vector": exp_vector, "text": exp_text},
            }
    ...
    query_types_temporal = set(
        list(control_temporal_factors.keys()) + list(exp_temporal_factors.keys())
    )
    ...
    for model in all_models:
        ...
        if control_active != exp_active or control_weight != exp_weight:
            model_diffs[model] = {
                "control": {"is_active": control_active, "weight": control_weight},
                "experimental": {"is_active": exp_active, "weight": exp_weight},
            }
...
def display_settings_comparison(settings_path: str, golden_queries_path: str) -> None:
    ...
    for control_key, exp_key in basic_settings:
        ...
        print(
            f"{control_key:<30}|{str(control_value):<25}|{str(exp_value):<25}{highlight}"
        )
    ...
    print("-" * 80)
    print(
        f"{'Query Type':<15}|{'Vector (Control)':<18}|{'Text (Control)':<18}|{'Vector (Exp)':<18}|{'Text (Exp)':<18}"
    )
    print("-" * 80)
    ...
    for query_type in sorted(query_types):
        ...
        highlight = (
            " *" if control_vector != exp_vector or control_text != exp_text else ""
        )
        print(
            f"{query_type:<15}|{str(control_vector):<18}|{str(control_text):<18}|{str(exp_vector):<18}|{str(exp_text):<18}{highlight}"
        )
    ...
    print("-" * 80)
    print(
        f"{'Query Type':<15}|{'Boost Factor (Control)':<25}|{'Boost Factor (Exp)':<25}"
    )
    print("-" * 80)
    ...
    for query_type in sorted(query_types_temporal):
        ...
        print(
            f"{query_type:<15}|{str(control_factor):<25}|{str(exp_factor):<25}{highlight}"
        )
    ...
    print("-" * 80)
    print(
        f"{'Model':<20}|{'Status (Control)':<15}|{'Weight (Control)':<15}|{'Status (Exp)':<15}|{'Weight (Exp)':<15}"
    )
    print("-" * 80)
    ...
    for model in sorted(all_models):
        ...
        # Highlight differences
        highlight = (
            " *" if control_active != exp_active or control_weight != exp_weight else ""
        )
        print(
            f"{model:<20}|{str(control_active):<15}|{str(control_weight):<15}|{str(exp_active):<15}|{str(exp_weight):<15}{highlight}"
        )
...
def sync_experimental_to_control(settings_path: str, golden_queries_path: str) -> bool:
    ...
    try:
        ...
        if not create_backup(golden_queries_path):
            logger.warning(
                f"Failed to create backup of {golden_queries_path}, proceeding with caution"
            )
        ...
        if "retrieval" in memnon_settings:
            golden_queries_content["settings"]["retrieval"] = copy.deepcopy(
            ...
            if (
                ...
                in golden_queries_content["settings"]["retrieval"]
            ):
                golden_queries_content["settings"]["structured_data"] = (
                    golden_queries_content["settings"]["retrieval"][
                        "structured_data_enabled"
                    ]
                )
            ...
            if "hybrid_search" in memnon_settings["retrieval"]:
                if (
                    "weights_by_query_type"
                    in memnon_settings["retrieval"]["hybrid_search"]
                ):
                    golden_queries_content["settings"]["retrieval"]["hybrid_search"][
                        "weights_by_query_type"
                    ] = copy.deepcopy(
        ...
        if "models" in memnon_settings:
            golden_queries_content["settings"]["models"] = copy.deepcopy(
                memnon_settings["models"]
            )
...
def sync_control_to_experimental(settings_path: str, golden_queries_path: str) -> bool:
    ...
    try:
        ...
        if not create_backup(settings_path):
            logger.warning(
                f"Failed to create backup of {settings_path}, proceeding with caution"
            )
        ...
        if "retrieval" in experimental_settings:
            ...
            if "structured_data" in experimental_settings:
                exp_retrieval["structured_data_enabled"] = experimental_settings[
                    "structured_data"
                ]
            ...
            if "hybrid_search" in experimental_settings["retrieval"]:
                if (
                    "weights_by_query_type"
                    in experimental_settings["retrieval"]["hybrid_search"]
                ):
                    exp_retrieval["hybrid_search"]["weights_by_query_type"] = (
                        copy.deepcopy(
                            experimental_settings["retrieval"]["hybrid_search"][
                                "weights_by_query_type"
                            ]
                        )
                    )
        ...
        if "models" in experimental_settings:
            settings_content["Agent Settings"]["MEMNON"]["models"] = copy.deepcopy(
                experimental_settings["models"]
            )
    ...
    try:
        ...
        if choice == "1":
            ...
            confirm = input(
                ...
                "A backup will be created as golden_queries.json.bak. Continue? (y/n): "
            ).lower()
            ...
            confirm = input(
                ...
                "A backup will be created as settings.json.bak. Continue? (y/n): "
            ).lower()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compare and manage NEXUS IR evaluation settings"
    )
    parser.add_argument(
        "--settings",
        type=str,
        default=os.path.join(parent_dir, "..", "settings.json"),
        help="Path to settings.json",
    )
    parser.add_argument(
        "--golden_queries",
        type=str,
        default=os.path.join(parent_dir, "golden_queries.json"),
        help="Path to golden_queries.json",
    )
    parser.add_argument(
        "--action",
        type=str,
        choices=["compare", "sync_to_control", "sync_to_experimental", "menu"],
        default="menu",
