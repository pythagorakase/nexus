...
def binary_preference(
    ...
    relevant_docs = {
        doc_id for doc_id, score in judgments.items() if score >= relevance_threshold
    }
    nonrelevant_docs = {
        doc_id for doc_id, score in judgments.items() if score < relevance_threshold
    }
        ...
        if doc_id in relevant_docs:
            ...
            nonrel_before = len(
                [
                    r
                    for r in range(rank)
                    if str(results[r].get("id", "")) in nonrelevant_docs
                ]
            )
...
def ndcg_at_k(
    results: List[Dict[str, Any]], judgments: Dict[str, int], k: int
) -> float:
    """
...
def calculate_all_metrics(
    results: List[Dict[str, Any]], judgments: Dict[str, int]
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "bpref": bpref,
        "ndcg@10": ndcg_10,
        "judged_counts": {"p@5": judged_5, "p@10": judged_10, "total": judged_total},
...
def average_metrics_by_category(
    query_metrics: Dict[str, Dict[str, Any]], query_categories: Dict[str, str]
    ...
    metric_sums = {
        ...
        "ndcg@10": 0.0,
        "unjudged_count": 0,
        "judged_counts": {"p@5": 0, "p@10": 0, "total": 0},
    }
    ...
    for query_text, metrics in query_metrics.items():
        ...
        for count_name in ["p@5", "p@10", "total"]:
            category_sums[category]["judged_counts"][count_name] += judged_counts.get(
                count_name, 0
            )
        ...
        if count > 0:
            for metric in ["p@5", "p@10", "mrr", "bpref", "ndcg@10"]:
                result["by_category"][category][metric] = (
                    category_sums[category][metric] / count
                )

            result["by_category"][category]["unjudged_count"] = category_sums[category][
                "unjudged_count"
            ]
            result["by_category"][category]["judged_counts"] = category_sums[category][
                "judged_counts"
            ]
