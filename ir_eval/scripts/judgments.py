...
def get_guidelines(
    golden_queries: Dict[str, Any], query_name: str, query_category: str
    ...
    if (
        query_category in golden_queries
        and query_name in golden_queries[query_category]
    ):
...
def judge_results_interactive(
    ...
    if not golden_queries:
        logger.warning(
            f"No golden queries found in {golden_queries_file}, continuing without guidelines"
        )
    ...
    for query_data in query_results:
        ...
        for i, result in enumerate(results, 1):
            ...
            # Add judgment
            qrels.add_judgment(query_text, doc_id, relevance, category, text)
            judgments_added += 1
...
def judge_run_from_db(
    run_id: int,
    qrels_file: str = None,
    golden_queries_file: str = None,
    db_path: str = None,
):
    """
...
""",
    ...
    source_group.add_argument(
        "--run-id", type=int, help="Database ID of the run to judge"
    )
    source_group.add_argument(
        "--run-ids", help="Comma-separated list of run IDs to judge"
    )

    # Other arguments
    parser.add_argument(
        "--qrels", help="Path to the QRELS JSON file (default: database default)"
    )
    ...
    parser.add_argument(
        "--skip-judged",
        action="store_true",
        help="Skip already judged documents (for file judging)",
    )
    ...
    if args.file:
        ...
        # Single run judging
        judge_run_from_db(args.run_id, args.qrels, args.golden_queries, args.db)
    elif args.run_ids:
