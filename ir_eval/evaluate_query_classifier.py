...
try:
    ...
    print(
        "Error: Could not import MEMNON. Make sure the nexus package is in your PYTHONPATH."
    )
    sys.exit(1)
...
def main():
    ...
    for cat in labels:
        ...
        print(
            f"{cat:<15} {precision[cat]:.4f}{'':6} {recall[cat]:.4f}{'':6} {f1[cat]:.4f}{'':6} {'✓' if meets else '✗'}"
        )
    ...
    if all_meet_threshold:
        print(
            f"✓ All categories meet the ≥{threshold} threshold for precision and recall!"
        )
        print(
            "The classifier is robust enough to implement differential weights per query category."
        )
    else:
        print(
            f"✗ Not all categories meet the ≥{threshold} threshold for precision and recall."
        )
        print(
            "Consider improving the classifier patterns before implementing differential weights."
        )
