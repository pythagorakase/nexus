...
try:
    ...
    from ir_eval.scripts.ir_metrics import (
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "ir_eval.log")
        ),
    ...
    try:
        ...
        try:
            ...
            if not run_description:
                run_description = (
                    f"Generated by ir_eval.py with {config_type} configuration"
                )
...
def create_variations_file(
    original_data: Dict[str, Any], category: str, variations: List[Dict[str, Any]]
) -> str:
    """
...
def create_variations_file_all_categories(
    original_data: Dict[str, Any], all_variations: List[Dict[str, Any]]
) -> str:
    """
            ...
            while i < len(results):
                ...
                if vector_score is not None:
                    try:
                        ...
                        logger.warning(
                            f"Could not convert vector_score {vector_score} to float"
                        )
                        vector_score = 0.0
                else:
                    vector_score = 0.0
                ...
                if text_score is not None:
                    try:
                        ...
                        logger.warning(
                            f"Could not convert text_score {text_score} to float"
                        )
                        text_score = 0.0
                else:
                    text_score = 0.0
                ...
                score_display += (
                    f" (Vector: {vector_score:.4f}, Text: {text_score:.4f})"
                )
                ...
                while True:
                    ...
                    try:
                        ...
                        if 0 <= relevance <= 3:
                            # Add judgment
                            qrels.add_judgment(
                                query_text, doc_id, relevance, category, text
                            )
                            ...
                            if is_variation:
                                ...
                                original_name = query_data.get("name", "").replace(
                                    " - Variation", ""
                                )

                                # Find the original query in golden_queries
                                for cat in self.golden_queries:
                                    if cat != "settings" and isinstance(
                                        self.golden_queries[cat], dict
                                    ):
                                        if original_name in self.golden_queries[cat]:
                                            original_query = self.golden_queries[cat][
                                                original_name
                                            ].get("query", "")
                                            break
                            ...
                            if (
                                original_query
                                and variation_query
                                and original_query != variation_query
                            ):
                                # Determine which one to copy to
                                target_query = (
                                    original_query if is_variation else variation_query
                                )
                                print(
                                ...
                                qrels.add_judgment(
                                    target_query, doc_id, relevance, category, text
                                )
...
def evaluate_run(
    run_id: int, qrels: QRELSManager, db: Optional[IRDatabase] = None
) -> Dict[str, Any]:
    """
...
def compare_runs(
    run_ids: List[int], run_names: List[str], db: Optional[IRDatabase] = None
) -> Dict[str, Any]:
    """
...
class IREvalCLI:
    ...
    def run_query_variations(self):
        ...
        if not query_variations:
            ...
            print(
                "To use this feature, add a 'query_variation' field next to 'query' fields."
            )
            return
        ...
        total_variations = sum(
            len(variations) for variations in query_variations.values()
        )
        ...
        if choice == 0:
            ...
            variations_file = create_variations_file_all_categories(
                self.golden_queries, all_variations
            )
        ...
        variations_file = create_variations_file(
            self.golden_queries, category, variations
        )
    ...
    def compare_results(self):
        ...
        try:
            ...
            if 1 <= pair_choice <= len(pairs):
                ...
                run_names = [
                    pair.get("control_name", "Control"),
                    pair.get("experiment_name", "Experiment"),
                ]
            else:
                print("Invalid choice. Please try again.")
                return
    ...
    def _compare_query_variations(self, query_variation_runs):
        ...
        for query_data in query_results:
            ...
            if " - Variation" in query_name:
                ...
                for category in self.golden_queries:
                    if category != "settings" and isinstance(
                        self.golden_queries[category], dict
                    ):
                        if original_name in self.golden_queries[category]:
                            original_query = self.golden_queries[category][
                                original_name
                            ].get("query", "")
                            break
            ...
            if original_query:
                ...
                if original_query not in variations_by_original:
                    variations_by_original[original_query] = {
                        "original": {"query": original_query, "metrics": None},
                        "variation": {"query": variation_query, "metrics": None},
                    }
        ...
        if original_metrics["count"] > 0 and variation_metrics["count"] > 0:
            ...
            print("\n" + "=" * 80)
            print(
                f"SUMMARY METRICS (Averaged across {original_metrics['count']} query pairs)"
            )
            print("=" * 80)
    ...
    def delete_runs(self):
        ...
        if choice.upper() == "A":
            ...
            confirm = input(
                "Are you sure you want to delete ALL runs? This cannot be undone. (y/n): "
            )
            if confirm.lower() != "y":
                print("Deletion cancelled")
                return
        ...
        try:
            ...
            confirm = input(
                f"Are you sure you want to delete run(s) {id_list}? This cannot be undone. (y/n): "
            )
            if confirm.lower() != "y":
                print("Deletion cancelled")
                return
            ...
            print(
                "Invalid input. Please enter comma-separated run IDs or 'A' for all runs"
            )
    ...
    def _delete_specific_runs(self, run_ids):
        ...
        try:
            ...
            cursor.execute(
                f"DELETE FROM comparisons WHERE best_run_id IN ({ids_str})", run_ids
            )
