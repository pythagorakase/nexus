...
def main():
    ...
    control_settings = {
        "retrieval": {
            ...
            "models": {"bge-small": {"is_active": True, "weight": 1.0}},
        }
    }
    ...
    experiment_settings = {
        "retrieval": {
            ...
            "models": {
                "infly_inf-retriever-v1-1.5b": {"is_active": True, "weight": 1.0}
            },
        }
    }
    ...
    control_run_id = db.add_run(
        "Control", control_settings, "control", "Vector search only with bge-small"
    )
    experiment_run_id = db.add_run(
        "Experiment",
        experiment_settings,
        "experiment",
        "Hybrid search with inf-retriever",
    )
    ...
    for query_text, category, _ in queries:
        ...
        for i in range(10):
            ...
            qrels.add_judgment(
                ...
                relevance,
                category,
                f"Sample document {i+1} for {category}",
            )
    ...
    comparison_data = {
        "overall": {
            "p@5": {
                ...
                "best_run": (
                    "Experiment"
                    if experiment_metrics["aggregated"]["overall"]["p@5"]
                    > control_metrics["aggregated"]["overall"]["p@5"]
                    else "Control"
                ),
            },
            "p@10": {
                ...
                "best_run": (
                    "Experiment"
                    if experiment_metrics["aggregated"]["overall"]["p@10"]
                    > control_metrics["aggregated"]["overall"]["p@10"]
                    else "Control"
                ),
            },
            "mrr": {
                ...
                "best_run": (
                    "Experiment"
                    if experiment_metrics["aggregated"]["overall"]["mrr"]
                    > control_metrics["aggregated"]["overall"]["mrr"]
                    else "Control"
                ),
            },
            "bpref": {
                ...
                "best_run": (
                    "Experiment"
                    if experiment_metrics["aggregated"]["overall"]["bpref"]
                    > control_metrics["aggregated"]["overall"]["bpref"]
                    else "Control"
                ),
            },
    ...
    print(
        f"  P@5:  {experiment_metrics['aggregated']['overall']['p@5']:.4f} ({p5_diff:+.4f})"
    )
    print(
        f"  P@10: {experiment_metrics['aggregated']['overall']['p@10']:.4f} ({p10_diff:+.4f})"
    )
    print(
        f"  MRR:  {experiment_metrics['aggregated']['overall']['mrr']:.4f} ({mrr_diff:+.4f})"
    )
    print(
        f"  BPREF: {experiment_metrics['aggregated']['overall']['bpref']:.4f} ({experiment_metrics['aggregated']['overall']['bpref'] - control_metrics['aggregated']['overall']['bpref']:+.4f})"
    )
    print()
    print(
        f"Best Overall Run: {'Experiment' if best_run_id == experiment_run_id else 'Control'}"
    )
    print("-" * 70)
    ...
    for category in control_metrics["aggregated"]["by_category"]:
        ...
        print(
            f"  Control P@5:    {control_metrics['aggregated']['by_category'][category]['p@5']:.4f}"
        )
        print(
            f"  Experiment P@5: {experiment_metrics['aggregated']['by_category'][category]['p@5']:.4f} ({experiment_metrics['aggregated']['by_category'][category]['p@5'] - control_metrics['aggregated']['by_category'][category]['p@5']:+.4f})"
        )
        print()
