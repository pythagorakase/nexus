...
def analyze_query_original(query_text: str) -> Dict[str, Any]:
    ...
    query_info = {"text": query_text, "type": "general"}  # Default
    ...
    theme_patterns = [r"\btheme\b", r"\bmotif\b", r"\bsymbolism\b", r"\bmeaning\b"]
...
def evaluate_classifier(
    queries: List[Dict[str, Any]], analyzer_func
...
def main():
    ...
    original_precision, original_recall, original_f1, original_matrix = (
        evaluate_classifier(queries, analyze_query_original)
    )

    # Evaluate improved classifier
    improved_precision, improved_recall, improved_f1, improved_matrix = (
        evaluate_classifier(queries, analyze_query_improved)
    )
    ...
    original_meets = {
        cat: (
            original_precision[cat] >= threshold and original_recall[cat] >= threshold
        )
        for cat in labels
    }
    improved_meets = {
        cat: (
            improved_precision[cat] >= threshold and improved_recall[cat] >= threshold
        )
        for cat in labels
    }
    ...
    for cat in labels:
        ...
        print(
            f"{cat:<15} {original_f1[cat]:.4f}{'':6} {improved_f1[cat]:.4f}{'':6} {change_str}"
        )
    ...
    for cat in labels:
        print(f"\n{cat.upper()}:")
        print(
            f"  Precision: {original_precision[cat]:.4f} -> {improved_precision[cat]:.4f} ({improved_precision[cat] - original_precision[cat]:+.4f})"
        )
        print(
            f"  Recall:    {original_recall[cat]:.4f} -> {improved_recall[cat]:.4f} ({improved_recall[cat] - original_recall[cat]:+.4f})"
        )
        print(
            f"  F1 Score:  {original_f1[cat]:.4f} -> {improved_f1[cat]:.4f} ({improved_f1[cat] - original_f1[cat]:+.4f})"
        )
        print(
            f"  Meets ≥{threshold}: {'✓' if original_meets[cat] else '✗'} -> {'✓' if improved_meets[cat] else '✗'}"
        )
    ...
    print("\nOverall Assessment:")
    print(
        f"  Original classifier: {'✓' if all_original_meet else '✗'} {sum(original_meets.values())}/{len(labels)} categories meet threshold"
    )
    print(
        f"  Improved classifier: {'✓' if all_improved_meet else '✗'} {sum(improved_meets.values())}/{len(labels)} categories meet threshold"
    )
