...
DEFAULT_SQLITE_DB_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "ir_eval.db"
)
...
def migrate_queries(
    sqlite_conn: sqlite3.Connection, pg_db: IRDatabasePG
) -> Dict[int, int]:
    """
    ...
    try:
        ...
        for query in queries:
            pg_query_id = pg_db.add_query(
                query["text"], query["category"], query["name"]
            )
            if pg_query_id:
                query_id_map[query["id"]] = pg_query_id
                logger.debug(
                    f"Migrated query {query['id']} -> {pg_query_id}: {query['text'][:30]}"
                )
...
def migrate_judgments(
    sqlite_conn: sqlite3.Connection, pg_db: IRDatabasePG, query_id_map: Dict[int, int]
) -> int:
    """
    ...
    try:
        ...
        for judgment in judgments:
            ...
            if sqlite_query_id not in query_id_map:
                logger.warning(
                    f"Skipping judgment with unknown query ID: {sqlite_query_id}"
                )
                continue
...
def migrate_runs(
    ...
    try:
        ...
        sqlite_cursor = sqlite_conn.cursor()
        sqlite_cursor.execute(
            "SELECT id, name, timestamp, settings, description, config_type FROM runs"
        )
        ...
        for run in runs:
            ...
            pg_run_id = pg_db.add_run(
                run["name"], settings, run["config_type"], run["description"]
            )
            ...
            for result in results:
                ...
                if sqlite_query_id not in query_id_map:
                    logger.warning(
                        f"Skipping result with unknown query ID: {sqlite_query_id}"
                    )
                    continue
                ...
                if sqlite_query_id not in query_results:
                    # Get query info
                    sqlite_cursor.execute(
                        "SELECT text, category, name FROM queries WHERE id = ?",
                        (sqlite_query_id,),
                    )
                    ...
                    if not query_info:
                        logger.warning(
                            f"Could not find query info for ID: {sqlite_query_id}"
                        )
                        continue
                ...
                try:
                    ...
                    logger.warning(
                        f"Skipping result with invalid chunk_id: {result['doc_id']}"
                    )
                    continue
            ...
            if query_results:
                ...
                if success:
                    logger.info(
                        f"Migrated {sum(len(q['results']) for q in query_results.values())} results for run {pg_run_id}"
                    )
                else:
        ...
        for pair in run_pairs:
            if (
                pair["control_run_id"] in run_id_map
                and pair["experiment_run_id"] in run_id_map
            ):
                pg_control_id = run_id_map[pair["control_run_id"]]
                pg_experiment_id = run_id_map[pair["experiment_run_id"]]

                pg_db.link_run_pair(
                    pg_control_id, pg_experiment_id, pair["description"]
                )
        ...
        for sqlite_run_id, pg_run_id in run_id_map.items():
            ...
            for metrics in metrics_rows:
                ...
                if sqlite_query_id not in query_id_map:
                    logger.warning(
                        f"Skipping metrics with unknown query ID: {sqlite_query_id}"
                    )
                    continue
...
def migrate_comparisons(
    sqlite_conn: sqlite3.Connection, pg_db: IRDatabasePG, run_id_map: Dict[int, int]
) -> int:
    """
    ...
    try:
        ...
        for comp in comparisons:
            ...
            comparison_data = (
                json.loads(comp["comparison_data"]) if comp["comparison_data"] else {}
            )
            ...
            if None in pg_run_ids:
                logger.warning(
                    f"Skipping comparison {comp['id']} due to unmapped run IDs"
                )
                continue

            # Map best run ID
            pg_best_run_id = (
                run_id_map.get(comp["best_run_id"]) if comp["best_run_id"] else None
            )

            # Save comparison to PostgreSQL
            pg_comp_id = pg_db.save_comparison(
                pg_run_ids, run_names, comparison_data, pg_best_run_id
            )
