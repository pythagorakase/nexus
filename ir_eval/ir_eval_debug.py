...
def create_temp_settings_file(
    ...
    if (
        ...
        logger.debug(
            f"Base settings before override: {json.dumps(settings_copy['Agent Settings']['MEMNON'].get('retrieval', {}).get('hybrid_search', {}), indent=2)}"
        )
        logger.debug(
            f"Override settings to apply: {json.dumps(memnon_override.get('retrieval', {}).get('hybrid_search', {}), indent=2)}"
        )
            ...
            if key in settings_copy["Agent Settings"]["MEMNON"]:
                if isinstance(value, dict) and isinstance(
                    ...
                    if key == "models":
                        for model_name, model_config in value.items():
                            if (
                                ...
                                if local_path:
                                    ...
                                    logger.debug(
                                        f"Preserved local_path for model {model_name}: {local_path}"
                                    )
                            else:
                                settings_copy["Agent Settings"]["MEMNON"]["models"][
                                    model_name
                        ...
                        pre_update = json.dumps(
                            settings_copy["Agent Settings"]["MEMNON"][key]
                        )
                        ...
                        logger.debug(
                            f"Updated {key} settings: {pre_update} -> {post_update}"
                        )
                else:
                    # Replace value
                    prev_value = settings_copy["Agent Settings"]["MEMNON"].get(
                        key, "None"
                    )
        ...
        logger.debug(
            f"Final experimental settings with hybrid_search config: "
    ...
    with open(temp_path, "r") as f:
        ...
        hybrid_settings = (
            debug_settings.get("Agent Settings", {})
            .get("MEMNON", {})
            .get("retrieval", {})
            .get("hybrid_search", {})
        )
        logger.debug(
            f"Temporary settings file hybrid search content: {hybrid_settings}"
        )
...
def test_run_golden_queries():
    ...
    if "settings" in golden_queries_data:
        ...
        if "retrieval" in golden_queries_data["settings"]:
            experimental_settings["retrieval"] = golden_queries_data["settings"][
                "retrieval"
            ]
    ...
    try:
        ...
        temp_settings_path = create_temp_settings_file(
            settings_data, experimental_settings
        )
        ...
        with open(temp_settings_path, "r") as f:
            ...
            hybrid_settings = (
                temp_settings.get("Agent Settings", {})
                .get("MEMNON", {})
                .get("retrieval", {})
                .get("hybrid_search", {})
            )
