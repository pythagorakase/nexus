...
def validate_table_counts(connection):
    ...
    for model in large_models:
        if "bge-small" in model.lower():
            logger.warning(
                f"WARNING: Small model '{model}' found in chunk_embeddings (1024D) table"
            )
    ...
    for model in small_models:
        if "bge-small" not in model.lower() and "bge-base" not in model.lower():
            logger.warning(
                f"WARNING: Large model '{model}' found in chunk_embeddings_small (384D) table"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate embedding tables and models."
    )
