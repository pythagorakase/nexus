...
def resolve_unknown_character(
    char_name: str, char_lookup: Dict[str, int], conn
) -> Optional[int]:
    """
        ...
        while True:
                ...
                for char_ref in characters:
                    ...
                    if char_name in resolved_mappings:
                        ...
                        if (
                            not char_id
                            and interactive
                            and char_name not in stats["unknown_characters"]
                        ):
                            resolved_id = resolve_unknown_character(
                                char_name, char_lookup, conn
                            )
                            if resolved_id:
                    ...
                    if char_id:
                        ...
                        insert_data.append(
                            (
                                ...
                                (
                                    ref_type
                                    if ref_type in ["present", "mentioned"]
                                    else None
                                ),
                            )
                        )
                        ...
                        logger.debug(
                            f"Unknown character: '{char_name}' in chunk {chunk_id}"
                        )
            ...
            if insert_data:
                try:
                    ...
                    conn.commit()
                    logger.info(
                        f"Inserted {len(insert_data)} references (offset: {offset})"
                    )
            ...
            if stats["processed_chunks"] % 100 == 0:
                ...
                logger.info(
                    f"Progress: {stats['processed_chunks']}/{stats['total_chunks']} ({pct:.1f}%)"
                )
...
def print_migration_report(stats: dict):
    ...
    print(
        f"Failed references: {stats['total_references'] - stats['inserted_references']}"
    )
    ...
    if stats["unknown_characters"]:
        ...
        sorted_unknown = sorted(
            stats["unknown_characters"].items(), key=lambda x: x[1], reverse=True
        )
    ...
    try:
        with get_db_connection() as conn:
            ...
            with conn.cursor() as cur:
                ...
                if existing_count > 0:
                    response = input(
                        f"Table already contains {existing_count} references. Continue? (y/n): "
                    )
                    if response.lower() != "y":
