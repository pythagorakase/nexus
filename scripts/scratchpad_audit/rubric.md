# Skald–Gaia Correspondence Rubric (v2.2, Anchored)
...
Scale lineage, for cross-run comparison: v1 (0–2) saturated — 4 of 6 dimensions ceilinged at 100%. v2 (0–3) fixed the ceiling but pinned 71–88% of exchanges at exactly 2. v2.1 splits that catch-all: **v2's 2 becomes v2.1's 2 (competent) or 3 (rich); v2's 3 becomes v2.1's 4 (exceptional).** Compare across versions at the rung level, not by means — no distributional mapping between versions is defined. v2.2 moves no rung boundaries; it codifies the twelve scoring conventions surfaced by the 2026-07-30 fresh-corpus validation run (`temp/rescore_2026-07-30_194151Z/`), so v2.1 and v2.2 scores compare directly.
...
4. **On-path cap.** "On-path" means play followed the line the *letters* prepared — the letters are the instrument under audit, so deviation is measured against the correspondence's own plan, not against whichever option the prose foregrounded. On-path exchanges cap Adaptability at 3 (rich proactive retirement of stale material); 4 requires a genuine deviation or loss converted into new architecture. Do not dock an on-path exchange below 2 merely for being on-path. Adaptation is credited at the exchange whose letters *react* to the deviation: when play deviates on one accepted chunk and the letters respond on the next, the reacting exchange carries the score (the deliberate mirror of Rule 6, which credits gifts where *given*).
...
- **4 — Architecture.** Commits to long-range structure with named payoff conditions or refusals — what must stay unresolved, what it converts into, why. "Long-range" means consequences extending beyond the immediate next beat: a durable refusal or a payoff condition that outlives the coming scene. *Anchor (chunk 14): the isolation method specified as "real but intentionally non-clean," all three future costs enumerated before the scene runs. Anchor (chunk 24): the killed terminal declared to convert "remote pressure into an imminent physical threshold."*
...
**Boundary rule (2 vs 3 vs 4):** conditions on the existing plan, or a bare decline = 2; conditions that reshape execution = 3; a new front, a traded aim, or a refusal with a substitute = 4. A new *instrument* inside an existing conflict — fresh machinery still serving the sent plan's purpose — reshapes execution and stays at 3. The three routes to 4 are: a **new front** (a stake or adversarial surface the sent plan did not contain — a new party engaged, a new thing put at risk), a **traded aim** (the returned element's role reverses or leaves its sent purpose — a liability handed back as a weapon), or a **refusal with a substitute**. The same-parties/same-stake test applies only to the new-front route; a traded aim or substitution can occur entirely inside the sent conflict. *(Rescore contrast: exchange 26's lie, sent as a liability to manage, came back "Bent:" as "a second vulnerability inside the witness circle" — traded aim, 4; exchange 34's consent sheet still served the sent consent question — machinery, 3; exchange 28's retraction sheet became a manipulable vulnerability testing the hall — new surface at risk, 4.)*
...
**Conditional and repeated promises:** first ask whether the exchange *newly* constrains. Restating a standing cost or deadline — including re-imposing a familiar cost structure on the next iteration of the same pressure after its first imposition was discharged — is recurrence and cannot earn 4, even when a branch carries "must." *(Rescore exchange 30: the authority-spending menu re-imposed after the deferral discharged exchange 29's binding constraint — 3.)* A genuinely new conditional binds when at least one branch carries a hard cost — "must," "whichever way," a named price honored if that branch occurs; "may" branches add color and neither bind nor disqualify. *(Rescore exchange 28: retreat left optional ("may") while escalation "must spend an office, signature, or overt authority" — binding.)* A promise whose every branch is optional ("may," "leave room for") binds nothing and caps at 3.

## Dimension 4: Continuity

Honoring the correspondence's own past. Continuity belongs to the conversation, not a seat: score the exchange pair — both letters together. (Story's first exchange: N/A per Rule 5.)
...
- **3 — Working memory.** Actively *uses* the past — an earlier promise or limit shapes this exchange's choices, beyond mere non-violation. Keeping an old obligation *live* is working memory: maintenance scores 3, however old the obligation.
- **4 — Long memory under pressure.** Pays off a promise at least five exchanges old, or holds a boundary in a scene that actively invites breaking it. Payoff means realized completion, not maintenance; "actively invites" requires an explicit live attempt, false claim, or temptation to cross the boundary — an ambient name or opportunity is not enough. *Anchor (chunk 16): chunk 11's "witness, not command" paid off five exchanges on and hardened. Anchor (chunk 19): Ren's exchange-7 undertaking cashed twelve exchanges later, verified in prose.*
