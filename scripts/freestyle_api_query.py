...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("freestyle_query.log"), logging.StreamHandler()],
)
...
def get_character_episode_spans(
    ...
    for span in episode_spans:
        logger.info(
            f"S{span['season']:02d}E{span['episode']:02d}: chunks {span['start_chunk']}-{span['end_chunk']}"
        )
...
def get_episode_chunks(
    engine: sa.engine.Engine, season: int, episode: int
) -> List[Dict[str, Any]]:
    """
...
def save_api_package(
    system_prompt: str, user_prompt: str, content: str, filename: str
) -> None:
    """
...
def main():
    ...
    parser.add_argument(
        "--prompt",
        help="Prompt text to send to the API (required unless using --input)",
    )
    ...
    # Place option
    parser.add_argument(
        "--place", type=int, help="Place ID to include place data as context"
    )
    ...
    if args.episode:
        ...
        if not place_context:
            ...
            base_context = prepare_summaries_context(
                season_summaries, episode_summaries
            )
    ...
    system_prompt = load(PromptId.OPERATORS_FREESTYLE_SYSTEM)

    # Define the user prompt without context
    user_prompt = load(PromptId.OPERATORS_FREESTYLE_USER, ARGS_PROMPT=f"{args.prompt}")
