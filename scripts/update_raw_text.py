...
class ChunkUpdater:
    ...
    def __init__(
        self, db_url: str = None, create_backup: bool = True, dry_run: bool = False
    ):
        """
    ...
    def find_chunk_by_scene_tag(
        self, session: Session, scene_tag: str
    ) -> Optional[NarrativeChunk]:
        """
        ...
        result = session.execute(
            query, {"season": season, "episode": episode, "scene": scene}
        ).first()

        if result:
            # Convert the result to a NarrativeChunk object
            chunk = NarrativeChunk(
                id=result[0], raw_text=result[1], created_at=result[2]
            )
            return chunk
    ...
    def update_chunk_raw_text(
        self, session: Session, chunk: NarrativeChunk, new_text: str
    ) -> bool:
        """
        ...
        if self.dry_run:
            logger.info(
                f"DRY RUN: Would update chunk {chunk.id} with new text ({len(new_text)} characters)"
            )
            return True
        ...
        try:
            ...
            logger.info(
                f"Updated chunk {chunk.id} with new text ({len(new_text)} characters)"
            )
            return True
    ...
    def find_insertion_point(
        ...
        if episode_chunks == 0:
            ...
            if available_space >= needed_space:
                ...
                logger.warning(
                    f"Not enough space for S{season:02d}E{episode:02d} (need {needed_space}, have {available_space})"
                )
    ...
    def reorganize_chunk_ids(
        self, session: Session, season: int, episode: int, needed_space: int
    ) -> bool:
        """
        ...
        if self.dry_run:
            logger.info(
                f"DRY RUN: Would reorganize IDs to make space for {needed_space} chunks in S{season:02d}E{episode:02d}"
            )
            return True
        ...
        try:
            ...
            logger.info(
                f"Reorganizing IDs: shifting all IDs >= {start_id} by +{shift_amount}"
            )
            ...
            logger.info(
                f"Using ON UPDATE CASCADE to shift IDs >= {start_id} by +{shift_amount}"
            )
            ...
            """
                ...
                {
                    "temp_shift": temp_start_id,
                    "shift_amount": shift_amount,
                    "temp_start_id": temp_start_id + start_id,
                },
            )
    ...
    def create_new_chunk(
        self, session: Session, scene_tag: str, raw_text: str, all_scene_tags: List[str]
    ) -> Optional[NarrativeChunk]:
        """
        ...
        try:
            ...
            if needs_reorganization:
                ...
                needed_space = self.count_new_chunks(all_scene_tags, season, episode)
                if not self.reorganize_chunk_ids(
                    session, season, episode, needed_space
                ):
                    logger.error(
                        f"Failed to reorganize chunk IDs for S{season:02d}E{episode:02d}"
                    )
                    return None
            ...
            logger.info(
                f"Slug will be set automatically by database trigger for chunk {chunk.id}"
            )

            logger.info(
                f"Created new chunk with ID {chunk.id} for scene tag {scene_tag}"
            )
            return chunk
                    ...
                    with self.Session() as session:
                        try:
                            ...
                            if chunk:
                                ...
                                logger.info(
                                    f"Chunk for scene tag {scene_tag} not found, creating new chunk"
                                )
                                new_chunk = self.create_new_chunk(
                                    session, scene_tag, new_text, all_scene_tags
                                )
                                if new_chunk:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update raw text in narrative chunks while preserving metadata."
    )

    parser.add_argument(
        "file_pattern",
        nargs="?",
        default="transcripts/*.md",
    ...
    updater = ChunkUpdater(
        db_url=args.db_url, create_backup=args.backup, dry_run=args.dry_run
    )
