...
def parse_arguments() -> argparse.Namespace:
    ...
    for action in parser._actions:
        if action.dest == "reasoning_effort":
            action.default = (
                "medium"  # Default to medium for structured output consistency
            )
    ...
    selection_method.add_argument(
        "--all", action="store_true", help="Process all chunks needing character data"
    )
        ...
        with db.connect() as conn:
            ...
            for row in result:
                ...
                characters[char.name.lower()] = (
                    char  # Store by lowercase name for easy lookup
                )
        ...
        with db.connect() as conn:
            if start_id is not None and end_id is not None:
                ...
                if overwrite:
                    ...
                    results = conn.execute(query).fetchall()
                    logger.info(
                        f"Found {len(results)} chunks needing character processing."
                    )
...
def format_known_characters_for_prompt(
    ...
    unique_chars = {
        char.id: char for char in known_characters.values()
...
def build_character_extraction_prompt(
    chunk: NarrativeChunk, known_characters_formatted: str
) -> str:
    """Build the prompt for the LLM to extract characters."""
    # Using the template from process_characters.md
    prompt = load(
        PromptId.OPERATORS_PROCESS_CHARACTERS,
        KNOWN_CHARACTERS_FORMATTED=f"{known_characters_formatted}",
        CHUNK_ID=f"{chunk.id}",
        CHUNK_RAW_TEXT=f"{chunk.raw_text}",
        CHUNK_ID_4=f"{chunk.id}",
    )
    return prompt


def get_llm_structured_response(
    prompt: str, provider_instance: LLMProvider, verbose: bool = False
) -> Optional[Dict[str, Any]]:
    """Calls the LLM provider ensuring structured JSON output."""
    if not isinstance(provider_instance, OpenAIProvider):
        logger.warning(
            f"Attempting structured output with non-OpenAI provider ({provider_instance.provider_name}). This may be less reliable."
        )
    ...
    try:
        ...
        supports_responses_api = provider_instance.model.startswith(
            "o"
        ) or provider_instance.model.startswith("gpt-4")
        ...
        if supports_responses_api:
            ...
            if provider_instance.system_prompt:
                input_messages.insert(
                    0, {"role": "system", "content": provider_instance.system_prompt}
                )
            ...
            if is_reasoning_model:
                api_params["reasoning"] = {
                    "effort": provider_instance.reasoning_effort or "medium"
                }
            ...
            if verbose:
                logger.debug(
                    f"Sending structured request to OpenAI responses API. Params (excluding input): { {k:v for k,v in api_params.items() if k != 'input'} }"
                )
            ...
            if hasattr(oai_response, "output_text") and oai_response.output_text:
                try:
                    ...
                    logger.info(
                        f"Successfully received structured JSON output from OpenAI."
                    )
                    # Optionally log reasoning if verbose and available
                    if (
                        verbose
                        and hasattr(oai_response, "reasoning")
                        and oai_response.reasoning
                    ):
                        if (
                            hasattr(oai_response.reasoning, "thinking")
                            and oai_response.reasoning.thinking
                        ):
                            logger.debug(
                                f"Reasoning/Thinking: {oai_response.reasoning.thinking[:500]}..."
                            )
                        elif (
                            hasattr(oai_response.reasoning, "summary")
                            and oai_response.reasoning.summary
                        ):
                            logger.debug(
                                f"Reasoning Summary: {oai_response.reasoning.summary}"
                            )
                    return structured_data
                except json.JSONDecodeError as e:
                    logger.error(
                        f"Failed to parse structured JSON output from OpenAI: {e}"
                    )
            ...
            logger.info(
                f"Model {provider_instance.model} might not support responses API. Trying chat completions with JSON mode."
            )
            ...
            try:
                chat_response = provider_instance.client.chat.completions.create(
                    ...
                    temperature=(
                        provider_instance.temperature
                        if provider_instance.temperature is not None
                        else 0.1
                    ),
                ...
                logger.info(
                    f"Successfully received JSON object output via chat completions."
                )
                return structured_data
            except Exception as e:
                ...
                if (
                    "chat_response" in locals()
                    and hasattr(chat_response, "choices")
                    and chat_response.choices
                ):
                    logger.debug(
                        f"Raw chat completion content: {chat_response.choices[0].message.content}"
                    )
                return None
        ...
        try:
            ...
            if is_reasoning_model and oai_response and hasattr(oai_response, "usage"):
                ...
                if reasoning_tokens is not None:
                    logger.info(
                        f"Token Usage (API): Input={input_tokens}, Output={output_tokens}, Reasoning={reasoning_tokens} for Chunk {structured_data.get('chunk_id', 'unknown')}"
                    )
                else:
                    # Fallback if reasoning_tokens attribute doesn't exist, log without it
                    logger.info(
                        f"Token Usage (API): Input={input_tokens}, Output={output_tokens} for Chunk {structured_data.get('chunk_id', 'unknown')}"
                    )
                    logger.warning(
                        "Could not retrieve reasoning_tokens from API response usage."
                    )
            else:
                ...
                output_tokens_est = get_token_count(
                    output_json_str, provider_instance.model
                )
                logger.info(
                    f"Token Usage (est.): Input={input_tokens_est}, Output={output_tokens_est} for Chunk {structured_data.get('chunk_id', 'unknown')}"
                )
        except Exception as e:
            logger.warning(
                f"Could not log token count for chunk {structured_data.get('chunk_id', 'unknown')}: {e}"
            )
...
def add_new_character(
    ...
    if char_name.lower() in known_characters or any(
        a.lower() in known_characters for a in aliases
    ):
        logger.warning(
            f"Character '{char_name}' marked as new by LLM but already exists in known characters. Skipping add."
        )
        # Return the existing character object
        return known_characters.get(char_name.lower()) or next(
            (
                known_characters[a.lower()]
                for a in aliases
                if a.lower() in known_characters
            ),
            None,
        )

    logger.info(
        f"Adding new character to database: Name='{char_name}', Aliases={aliases}"
    )
    try:
        with db.connect() as conn:
            trans = conn.begin()
            try:
                ...
                result = conn.execute(
                    insert_query, {"name": char_name, "aliases": aliases}
                ).fetchone()
                trans.commit()

                if result:
                    new_char = KnownCharacter(
                        id=result[0], name=result[1], aliases=result[2] or []
                    )
                    logger.info(
                        f"Successfully added character '{new_char.name}' with ID {new_char.id}"
                    )
                    ...
                    logger.error(
                        f"Failed to add character '{char_name}', INSERT returned no result."
                    )
                    return None
            except Exception as inner_e:
                logger.error(
                    f"Error during new character insertion transaction: {inner_e}"
                )
                trans.rollback()
                return None
    except Exception as e:
        logger.error(
            f"Database connection error while adding new character '{char_name}': {str(e)}"
        )
        return None


def add_aliases_to_character(
    db: Engine,
    ...
    logger.info(
        f"Adding aliases {new_aliases} to character '{character.name}' (ID: {character.id})"
    )
    ...
    try:
        with db.connect() as conn:
            ...
            try:
                ...
                result = conn.execute(
                    update_query, {"aliases": updated_aliases, "id": character.id}
                ).fetchone()
                trans.commit()
                ...
                if result:
                    ...
                    for old_alias in current_aliases:
                        if (
                            old_alias.lower() in known_characters
                            and known_characters[old_alias.lower()].id == character.id
                        ):
                    ...
                    logger.error(
                        f"Failed to update aliases for character '{character.name}', UPDATE returned no result."
                    )
                    return False
            except Exception as inner_e:
                logger.error(
                    f"Error during character alias update transaction: {inner_e}"
                )
                trans.rollback()
                return False
    except Exception as e:
        logger.error(
            f"Database connection error while updating aliases for character '{character.name}': {str(e)}"
        )
        return False
...
def update_chunk_metadata(
    ...
    mentioned_characters: List[Dict],
    dry_run: bool = False,
):
    """Updates the chunk_metadata.characters array for the given chunk."""
    all_characters_formatted = []
    processed_canonical_names = (
        set()
    )  # Ensure canonical names aren't added twice if present AND mentioned

    for char_list, status_suffix in [
        ...
        for char in char_list:
            ...
            if not canonical_name or char.get("status") == "new":
                canonical_name = char.get(
                    "name"
                )  # Use the name found in text for new chars

            if not canonical_name:
                logger.warning(
                    f"Character entry missing name and canonical_name in chunk {chunk_id}: {char}"
                )
                continue
            ...
            if canonical_name not in processed_canonical_names:
                ...
                status_suffix == "present"
                and f"{canonical_name}:mentioned" in all_characters_formatted
            ):
    ...
    logger.info(
        f"Updating chunk {chunk_id} metadata with characters: {all_characters_formatted}"
    )

    if dry_run:
        logger.info(
            f"[DRY RUN] Would update chunk {chunk_id} metadata characters to: {all_characters_formatted}"
        )
        return True
        ...
        with db.connect() as conn:
            ...
            try:
                ...
                if exists:
                    ...
                    conn.execute(
                        update_query,
                        {"characters": all_characters_formatted, "chunk_id": chunk_id},
                    )
                else:
                    ...
                    conn.execute(
                        insert_query,
                        {"chunk_id": chunk_id, "characters": all_characters_formatted},
                    )

                trans.commit()
                logger.info(
                    f"Successfully updated character metadata for chunk {chunk_id}."
                )
                return True
            except Exception as inner_e:
                logger.error(
...
def process_chunk(
    ...
    try:
        ...
        logger.info(
            f"Token Usage (est.): Input={input_tokens}, Output={output_tokens} for Chunk {chunk.id}"
        )
    except Exception as e:
    ...
    if not isinstance(present_chars, list) or not isinstance(mentioned_chars, list):
        logger.error(
            f"LLM output for chunk {chunk.id} has invalid format (present/mentioned not lists)."
        )
        return False
    ...
    for char_info in all_extracted_chars:
        if not isinstance(char_info, dict):
            logger.warning(
                f"Skipping invalid character entry in chunk {chunk.id}: {char_info}"
            )
            continue
        ...
        if status == "new":
            ...
            if not potential_match and char_info.get("aliases"):
                potential_match = next(
                    (
                        known_characters.get(a.lower())
                        for a in char_info["aliases"]
                        if a.lower() in known_characters
                    ),
                    None,
                )
            ...
            if potential_match:
                logger.warning(
                    f"LLM marked '{char_name}' as new, but matched existing character '{potential_match.name}'. Correcting."
                )
                ...
                existing_aliases = set(
                    alias.lower() for alias in potential_match.aliases
                )
                new_aliases = []
            ...
            if not canonical_name or canonical_name.lower() not in known_characters:
                logger.warning(
                    f"LLM provided unknown canonical_name '{canonical_name}' for known character '{char_name}' in chunk {chunk.id}. Attempting correction."
                )
                ...
                if match:
                    ...
                    logger.error(
                        f"Could not find match for 'known' character '{char_name}'. Treating as ambiguous."
                    )
                ...
                if character_obj:
                    existing_aliases = set(
                        alias.lower() for alias in character_obj.aliases
                    )
                    new_aliases = []
                    ...
                    if (
                        char_name.lower() != canonical_name.lower()
                        and char_name.lower() not in existing_aliases
                    ):
                        new_aliases.append(char_name)
                    ...
                    for alias in char_info.get("aliases", []):
                        if (
                            ...
                            and alias.lower() != canonical_name.lower()
                        ):
                            new_aliases.append(alias)
    ...
    if potential_new_chars and not dry_run:
        logger.info(
            f"Found {len(potential_new_chars)} potential new characters in chunk {chunk.id}."
        )

        # Process each potential new character interactively
        for i in range(
            len(potential_new_chars) - 1, -1, -1
            ...
            if char_name and char_name.lower() in blacklist:
                ...
                if char_info in present_chars:
                    present_chars.remove(char_info)
                if char_info in mentioned_chars:
                    mentioned_chars.remove(char_info)
            ...
            try:
                ...
                if user_input == "0":  # User rejected initial proposal
                    ...
                    if action_choice == "1":
                        ...
                        print(
                            f"Current aliases: {', '.join(current_aliases) if current_aliases else 'None'}"
                        )
                        new_aliases_input = input(
                            "New aliases (comma-separated) or enter to keep current: "
                        )
                        if new_aliases_input.strip():
                            new_aliases = [
                                a.strip()
                                for a in new_aliases_input.split(",")
                                if a.strip()
                            ]
                        else:
                        ...
                        retry_confirm = input(
                            "Add this edited character as NEW? [1=Yes, 0=No]: "
                        )
                        if retry_confirm != "0":
                            ...
                            new_char_obj = add_new_character(
                                db, char_info, known_characters
                            )
                            if new_char_obj:
                                char_info["canonical_name"] = (
                                    new_char_obj.name
                                )  # Update again with DB name
                                newly_added_chars.append(new_char_obj.name)
                            else:
                                logger.error(
                                    f"Failed to add edited new character '{new_name}'. Discarding entry."
                                )
                                if char_info in present_chars:
                                    present_chars.remove(char_info)
                                if char_info in mentioned_chars:
                        ...
                        logger.info(
                            f"Attempting to link '{char_name}' to an existing character."
                        )
                        if not known_characters:
                            logger.warning(
                                "No known characters to link to. Discarding."
                            )
                            if char_info in present_chars:
                                present_chars.remove(char_info)
                            if char_info in mentioned_chars:
                            ...
                            unique_chars_by_id = sorted(
                                list(set(known_characters.values())), key=lambda c: c.id
                            )
                            ...
                            while True:
                                try:
                                    link_choice_str = input(
                                        f"Enter character ID to link to: "
                                    )
                                    ...
                                    logger.warning(
                                        "Input aborted during linking. Discarding."
                                    )
                            ...
                            if valid_link:
                                ...
                                logger.info(
                                    f"Linking '{char_name}' to existing character '{selected_char.name}' (ID: {selected_char.id})."
                                )
                                ...
                                char_info["aliases"] = (
                                    []
                                )  # Clear aliases as they belonged to the rejected name
                                char_info.pop("_pending_confirmation", None)
                            else:
                                # Linking failed or was aborted
                                logger.warning(
                                    f"Linking failed or aborted for '{char_name}'. Discarding entry."
                                )
                                if char_info in present_chars:
                                    present_chars.remove(char_info)
                                if char_info in mentioned_chars:
                    ...
                    if new_char_obj:
                        ...
                        logger.error(
                            f"Failed to add new character '{char_name}'. Discarding entry."
                        )
                        if char_info in present_chars:
                            present_chars.remove(char_info)
                        if char_info in mentioned_chars:
                ...
                logger.warning(
                    "EOF detected during character confirmation. Discarding character."
                )
                if char_info in present_chars:
                    present_chars.remove(char_info)
                if char_info in mentioned_chars:
    ...
    if chars_with_new_aliases and not dry_run:
        logger.info(
            f"Found {len(chars_with_new_aliases)} characters with potential new aliases in chunk {chunk.id}."
        )
        ...
        for char_info in chars_with_new_aliases:
            ...
            for alias in original_aliases:
                if alias.lower() in blacklist:
                    logger.info(
                        f"Auto-rejecting blacklisted alias '{alias}' for character '{char_info.get('_existing_character').name}'."
                    )
                else:
            ...
            if not valid_aliases:
                logger.info(
                    f"All proposed aliases for '{char_info.get('_existing_character').name}' were blacklisted. Skipping review."
                )
            ...
            try:
                ...
                if user_input == "0":
                    ...
                    if action_choice == "1":
                        # Discard aliases
                        logger.info(
                            f"Discarding aliases '{new_aliases_str}' for character '{existing_character.name}'."
                        )
                        ...
                        new_aliases_input = (
                            input(
                                f"New aliases (comma-separated) [{new_aliases_str}]: "
                            )
                            or new_aliases_str
                        )
                        edited_aliases = [
                            a.strip() for a in new_aliases_input.split(",") if a.strip()
                        ]

                        # Retry confirmation
                        print(f"\nUpdated aliases: {', '.join(edited_aliases)}")
                        retry_confirm = input(
                            f"Add these aliases to character '{existing_character.name}'? [1=Yes, 0=No]: "
                        )
                ...
                logger.warning(
                    "EOF detected during alias confirmation. Defaulting to not adding aliases."
                )
                user_confirmed = False

            if user_confirmed:
                # Add the confirmed new aliases
                success = add_aliases_to_character(
                    db, existing_character, new_aliases, known_characters
                )
                if success:
                    logger.info(
                        f"Successfully added aliases {new_aliases} to character '{existing_character.name}'"
                    )
                else:
                    logger.error(
                        f"Failed to add aliases {new_aliases} to character '{existing_character.name}'"
                    )
            else:
                logger.info(
                    f"Skipping addition of aliases {new_aliases} to character '{existing_character.name}' based on user input."
                )
    ...
    for char_list in [present_chars, mentioned_chars]:
        ...
        for i in range(len(char_list) - 1, -1, -1):
            if char_list[i].get("_pending_confirmation", False):
                logger.warning(
                    f"Removing unconfirmed character '{char_list[i].get('name')}' from results."
                )
                char_list.pop(i)
    ...
    if newly_added_chars:
        logger.info(
            f"Added new characters in this chunk: {', '.join(newly_added_chars)}"
        )

    # 6. Update Chunk Metadata in Database
    update_success = update_chunk_metadata(
        db, chunk.id, present_chars, mentioned_chars, dry_run
    )
...
def main():
    ...
    try:
        ...
        raw_blacklist = (
            ...
            .get("process_characters", {})
            .get("blacklist", [])
        )
        if isinstance(raw_blacklist, list):
            blacklist = {
                item.lower() for item in raw_blacklist if isinstance(item, str)
            }
        ...
        logger.warning(
            f"Error loading character blacklist from config: {e}. Using empty blacklist."
        )
    ...
    if args.test:
        ...
        try:
            ...
            known_chars_formatted_test = format_known_characters_for_prompt(
                known_characters_test
            )
        except Exception as e:
        ...
        try:
            ...
            test_prompt = build_character_extraction_prompt(
                first_chunk, known_chars_formatted_test
            )
    ...
    if not known_characters and not args.dry_run:
        logger.warning(
            "No known characters loaded from the database. Processing will assume all characters are new initially."
        )
    ...
    try:
        ...
        if args.provider.lower() == "openai":
            provider_kwargs["reasoning_effort"] = (
                args.reasoning_effort or "medium"
            )  # Use arg or default medium
    ...
    for i in range(0, total_chunks, batch_size):
        ...
        for chunk in batch:
            ...
            within_limit, input_tokens, total_tokens = llm_provider.check_tpm_limit(
                temp_prompt, estimated_output_tokens=500
            )  # Estimate 500 output tokens for JSON

            if not within_limit:
                logger.error(
                    f"TPM limit exceeded for chunk {chunk.id}. Estimated tokens: {total_tokens}. Skipping chunk."
                )
                error_count += 1
                # Potentially wait or break if limits are consistently hit
                rate_limit_wait = COOLDOWNS.get("rate_limit", 300)
                logger.warning(
                    f"TPM Limit hit. Waiting {rate_limit_wait} seconds before trying next chunk..."
                )
