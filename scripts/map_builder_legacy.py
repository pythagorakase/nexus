...
try:
    ...
    print(
        "Error: Could not import from api_batch.py. Make sure it's in the same directory or Python path."
    )
    print(e)
    sys.exit(1)
...
class KnownPlace:
    ...
    def __init__(
        self, id: int, name: str, type: str, zone: int, summary: Optional[str] = None
    ):
        self.id = id
        self.name = name
        self.type = type
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.description = (
        "Extract location information from narrative chunks using an LLM."
    )
    ...
    selection_method = chunk_group.add_mutually_exclusive_group(
        required=False
    )  # Changed to not required so --test can work alone
    selection_method.add_argument("--start", type=int, help="Starting chunk id number")
    selection_method.add_argument(
        "--all", action="store_true", help="Process all chunks needing location data"
    )
    ...
    chunk_group.add_argument(
        "--end",
        type=int,
        help="Ending chunk id number (defaults to start if only start is provided)",
    )

    # Add processing control arguments (reuse from api_batch)
    process_group = next(
        (g for g in parser._action_groups if g.title == "Processing Options"), None
    )
    if process_group:
        process_group.add_argument(
            "--auto",
            action="store_true",
            help="Process all chunks automatically without prompting between batches",
        )
        process_group.add_argument(
            "--verbose",
            action="store_true",
            help="Print detailed information including prompts",
        )
        process_group.add_argument(
            "--thinking",
            action="store_true",
    ...
    if not args.test and not (args.start is not None or args.all):
        parser.error(
            "Either --test mode or a chunk selection method (--start or --all) is required"
        )
    ...
    try:
        with db.connect() as conn:
            ...
            try:
                ...
                # All checks passed
                logger.info(
                    "Database schema validation successful. Tables and relationships verified."
                )
                return True
            except Exception as e:
        ...
        with db.connect() as conn:
            if start_id is not None and end_id is not None:
                ...
                results = conn.execute(
                    query, {"start": start_id, "end": end_id}
                ).fetchall()
                logger.info(
                    f"Found {len(results)} chunks in range {start_id}-{end_id}."
                )
            else:
                ...
                if overwrite:
                    ...
                    logger.info(
                        f"Found {len(results)} total chunks for processing (overwrite=True)."
                    )
                else:
                    ...
                    results = conn.execute(query).fetchall()
                    logger.info(
                        f"Found {len(results)} chunks needing location processing."
                    )

            return [
                NarrativeChunk(id=row[0], raw_text=row[1], slug=row[2])
                for row in results
            ]
    except Exception as e:
...
def format_known_places_for_prompt(
    places: Dict[int, KnownPlace], zones: Dict[int, Zone]
) -> str:
    ...
    for zone_id, zone_places in sorted(places_by_zone.items()):
        # Get zone name
        zone_name = (
            zones[zone_id].name if zone_id in zones else f"Unknown Zone (ID: {zone_id})"
        )
        ...
        for place_id, place in sorted(zone_places):
            ...
            places_section += (
                f"├─── {place_id}: {place.name} ({place.type}){place_summary}\n"
            )
...
"""
    ...
    system_prompt = load(PromptId.OPERATORS_MAP_BUILDER)

    # JSON schema specification
    # Use double curly braces {{ }} to escape literal JSON braces in the f-string
    json_schema = load(PromptId.OPERATORS_MAP_BUILDER_SCHEMA, CHUNK_ID=f"{chunk.id}")
...
def get_llm_structured_response(
    ...
    try:
        ...
        if isinstance(provider_instance, AnthropicProvider) and hasattr(
            provider_instance.client, "messages"
        ):
            # Check that the chunk_id is valid
            if not chunk_id or chunk_id <= 0:
                logger.warning(
                    "Invalid chunk_id for caching, falling back to standard completion"
                )
                response = provider_instance.get_completion(prompt)
            else:
                if thinking_mode:
                    logger.info(
                        f"Using Anthropic with ephemeral caching and thinking mode for chunk {chunk_id}"
                    )
                else:
                    logger.info(
                        f"Using Anthropic with ephemeral caching for chunk {chunk_id}"
                    )
                ...
                if thinking_mode:
                    ...
                    thinking_config = SETTINGS.get("API Settings", {}).get(
                        "map_builder", {}
                    )

                    if (
                        "max_tokens" in thinking_config
                        and "budget_tokens" in thinking_config
                    ):
                        max_tokens_value = thinking_config["max_tokens"]
                        budget_tokens_value = thinking_config["budget_tokens"]
                        logger.info(
                            f"Thinking mode requested with max_tokens={max_tokens_value}, budget_tokens={budget_tokens_value}"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"Making direct API call to Anthropic with thinking mode enabled"
                            )
                            ...
                            if temp != 0:
                                logger.info(
                                    f"Omitting temperature parameter (was {temp}) when using thinking mode"
                                )
                            ...
                            logger.info(
                                f"API request payload structure: {json.dumps({k: '...' for k in payload.keys()}, indent=2)}"
                            )
                            if "system" in payload:
                                logger.info(
                                    f"System prompt length: {len(payload['system'])}"
                                )
                            ...
                            if verbose:
                                ...
                                for k, v in payload.items():
                                    if k == "messages":
                                        ...
                                        payload_log[k] = (
                                            v[:100] + "..." if len(v) > 100 else v
                                        )
                                    else:
                                        payload_log[k] = v

                                logger.debug(
                                    f"API request payload details: {json.dumps(payload_log, indent=2)}"
                                )
                            ...
                            with httpx.Client(timeout=120.0) as client:
                                ...
                                logger.info(
                                    f"Received API response with status code: {response_data.status_code}"
                                )
                                ...
                                if response_data.status_code != 200:
                                    error_msg = (
                                        f"API Error {response_data.status_code}: "
                                    )
                                    try:
                                        ...
                                        logger.error(
                                            f"Error response text: {response_data.text}"
                                        )
                                ...
                                logger.info(
                                    f"API response content structure: content array length: {len(result.get('content', []))} items"
                                )
                                for i, content_item in enumerate(
                                    ...
                                    if isinstance(content_item, dict):
                                        ...
                                        logger.info(
                                            f"Content item #{i}: type={content_type}"
                                        )
                                    else:
                                        logger.info(
                                            f"Content item #{i}: not a dictionary, type={type(content_item)}"
                                        )
                                ...
                                try:
                                    ...
                                    if verbose:
                                        ...
                                        if "content" in result:
                                            logger.debug(
                                                f"Content keys: {[c.get('type', type(c)) for c in result['content']]}"
                                            )
                                            if result["content"] and isinstance(
                                                result["content"][0], dict
                                            ):
                                                logger.debug(
                                                    f"Content[0] keys: {list(result['content'][0].keys())}"
                                                )
                                    ...
                                    if "content" in result and isinstance(
                                        ...
                                        if text_block and "text" in text_block:
                                            ...
                                            text_block and "value" in text_block
                                        ):  # Handle potential other key names
                                    ...
                                    if not found_text_block:
                                        logger.warning(
                                            "No 'text' block found in API response content list (thinking mode). Setting content to empty."
                                        )
                                    ...
                                    logger.debug(
                                        f"Raw API response: {json.dumps(result, indent=2)[:1000]}..."
                                    )
                                    ...
                                    fallback_content = (
                                        str(result)
                                        if result
                                        else "Error: Failed to parse API response"
                                    )
                                    response = LLMResponse(
                            ...
                            logger.warning(
                                f"Error using direct API call for thinking mode: {thinking_error}. Falling back to standard completion."
                            )
                            ...
                            logger.debug(
                                f"Thinking error details: {str(thinking_error)}"
                            )
                        ...
                        logger.warning(
                            "Thinking mode requested but max_tokens or budget_tokens not found in settings.json. Falling back to standard completion."
                        )
                        # Fall back to standard completion without caching
                        response = provider_instance.get_completion(prompt)
                        return response
                    ...
                    try:
                        ...
                        has_prev_chunk = (
                            "PREVIOUS CHUNK TEXT (FOR CONTEXT ONLY):" in prompt
                        )
                        ...
                        if len(system_parts) > 1:
                            ...
                            if "PREVIOUS CHUNK" in system_parts[1]:
                                places_and_schema = (
                                    schema_parts[0].strip() if schema_parts else ""
                                )
                            else:
                        ...
                        context_chunk_matches = re.findall(
                            r"CONTEXT CHUNK (\d+) \((.*?)\)", prompt
                        )
                                    ...
                                    if len(prev_parts) > 1:
                                        ...
                                        prev_chunks_text.append(
                                            (
                                                1,
                                                (
                                                    prev_chunk_text_parts[0].strip()
                                                    if prev_chunk_text_parts
                                                    else ""
                                                ),
                                            )
                                        )
                        ...
                        for chunk_id, chunk_text in sorted(
                            prev_chunks_text, key=lambda x: int(x[0])
                        ):
                        ...
                        if target_chunk_text:
                            target_header = (
                                f"TARGET CHUNK {target_id} TEXT:"
                                if target_id
                                else "TARGET CHUNK TEXT:"
                            )
                            system_messages.append(
                                {
                                    "type": "text",
                        ...
                        api_response = provider_instance.client.messages.create(
                            ...
                            messages=[
                                {
                                    "role": "user",
                                    "content": "Identify the location for this chunk using the provided information.",
                                }
                            ],
                        )
                    except Exception as cache_error:
                        logger.warning(
                            f"Error using ephemeral caching: {cache_error}. Falling back to standard completion."
                        )
                ...
                if verbose:
                    if thinking_mode:
                        logger.debug(
                            f"Thinking mode used. Token usage: {response.input_tokens} input, {response.output_tokens} output"
                        )
                        ...
                        logger.debug(
                            f"Ephemeral caching used. Token usage: {response.input_tokens} input, {response.output_tokens} output"
                        )
        else:
            ...
            if thinking_mode and not isinstance(provider_instance, AnthropicProvider):
                logger.warning(
                    "Thinking mode is only supported with Anthropic provider. Ignoring --thinking flag."
                )
            response = provider_instance.get_completion(prompt)
        ...
        try:
            ...
            logger.info(
                f"Response content to parse (first 100 chars): {content[:100]}..."
            )
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                try:
                    ...
                    if verbose:
                        logger.debug(
                            f"Parsed structured data: {json.dumps(structured_data, indent=2)}"
                        )

                    if not isinstance(structured_data, dict) or not structured_data.get(
                        "primary_location"
                    ):
                        logger.error(
                            f"Parsed JSON but missing required 'primary_location' field: {json.dumps(structured_data, indent=2)[:200]}..."
                        )
                        return None
                    ...
                    logger.error(
                        f"Invalid JSON string (first 100 chars): {json_str[:100]}..."
                    )
                    return None
            else:
                logger.error(
                    f"No valid JSON object found in LLM response (length: {len(content)})"
                )
                if "{" in content:
                    logger.error(
                        f"First '{' at position {start_idx}, last '}' at position {end_idx-1}"
                    )
                else:
            ...
            logger.error(
                f"Raw response content (first 200 chars): {response.content[:200]}..."
            )
            return None
        except Exception as ex:
            logger.error(f"Unexpected error parsing response: {ex}")
            logger.error(
                f"Response type: {type(response.content)}, length: {len(response.content)}"
            )
            ...
            if not response.content:
                ...
                logger.error(
                    f"Response content sample: {str(response.content)[:200]}..."
                )
            return None
    except Exception as e:
        logger.error(
            f"Error getting completion from {provider_instance.provider_name}: {e}"
        )
        return None
...
def add_new_place(
    ...
    if place_type not in valid_types:
        logger.warning(
            f"Invalid place type '{place_type}'. Using 'fixed_location' as fallback."
        )
    ...
    if zone_id not in zones:
        logger.warning(
            f"Invalid zone ID '{zone_id}' passed to add_new_place. This should have been caught earlier."
        )
        return None
    ...
    for place_id, place in places.items():
        if place.name.lower() == place_name.lower():
            logger.warning(
                f"Place '{place_name}' already exists with ID {place_id}. Returning existing record."
            )
            return place

    logger.info(
        f"Adding new place to database: Name='{place_name}', Type='{place_type}', Zone={zone_id}"
    )
    try:
        with db.connect() as conn:
            trans = conn.begin()
            try:
                ...
                existing_ids = [
                    row[0] for row in conn.execute(all_ids_query).fetchall()
                ]
                ...
                if existing_ids:
                    ...
                    for i in range(len(existing_ids)):
                        if (
                            i + 1 < len(existing_ids)
                            and existing_ids[i] + 1 < existing_ids[i + 1]
                        ):
                            # Found a gap
                ...
                while attempt <= max_attempts:
                    try:
                        ...
                        if "UniqueViolation" in str(
                            insert_error
                        ) and "places_pkey" in str(insert_error):
                            # ID is already used, try the next one
                            next_id += 1
                            logger.warning(
                                f"ID {next_id-1} already in use, trying ID {next_id} (attempt {attempt}/{max_attempts})"
                            )
                            attempt += 1
                        else:
                ...
                if attempt > max_attempts:
                    logger.error(
                        f"Failed to find an available ID after {max_attempts} attempts"
                    )
                    raise Exception(
                        f"Failed to find an available ID after {max_attempts} attempts"
                    )
                ...
                if result:
                    ...
                    logger.info(
                        f"Successfully added place '{new_place.name}' with ID {new_place.id}"
                    )
                    ...
                    logger.error(
                        f"Failed to add place '{place_name}', INSERT returned no result."
                    )
                    return None
        ...
        logger.error(
            f"Database connection error while adding new place '{place_name}': {str(e)}"
        )
        return None


def update_chunk_metadata(
    db: Engine, chunk_id: int, place_id: int, dry_run: bool = False
) -> bool:
    ...
    if dry_run:
        logger.info(
            f"[DRY RUN] Would update chunk {chunk_id} metadata place to: {place_id}"
        )
        return True
    ...
    try:
        with db.connect() as conn:
            ...
            try:
                ...
                check_query = text(
                    f"SELECT 1 FROM {CHUNK_METADATA_TABLE} WHERE chunk_id = :chunk_id"
                )
                exists = conn.execute(
                    check_query, {"chunk_id": chunk_id}
                ).scalar_one_or_none()
                ...
                if exists:
                    ...
                    conn.execute(
                        update_query, {"place": place_id, "chunk_id": chunk_id}
                    )
                else:
                    # Insert a new record if it doesn't exist
                    logger.warning(
                        f"No existing metadata found for chunk {chunk_id}. Inserting new record. Other fields might be default/null."
                    )
                    insert_query = text(
                        f"""
                    ...
                    conn.execute(
                        insert_query, {"chunk_id": chunk_id, "place": place_id}
                    )

                trans.commit()
                logger.info(
                    f"Successfully updated place metadata for chunk {chunk_id}."
                )
                return True
            except Exception as inner_e:
                logger.error(
                    f"Error during chunk metadata update transaction for chunk {chunk_id}: {inner_e}"
                )
                trans.rollback()
                return False
    except Exception as e:
        logger.error(
            f"Database connection error while updating metadata for chunk {chunk_id}: {str(e)}"
        )
        return False


def get_previous_chunks_info(
    db: Engine, current_chunk_id: int, num_chunks: int = 2
) -> List[Tuple[NarrativeChunk, Optional[KnownPlace]]]:
    ...
    try:
        with db.connect() as conn:
            ...
            prev_chunk_rows = conn.execute(
                prev_chunks_query,
                {"current_id": current_chunk_id, "num_chunks": num_chunks},
            ).fetchall()
            ...
            for prev_chunk_row in prev_chunk_rows:
                prev_chunk = NarrativeChunk(
                    id=prev_chunk_row[0],
                    raw_text=prev_chunk_row[1],
                    slug=prev_chunk_row[2],
                )
                ...
                if prev_place_id:
                    ...
                    place_row = conn.execute(
                        place_query, {"place_id": prev_place_id}
                    ).fetchone()
                    if place_row:
        ...
        logger.error(
            f"Error getting previous chunks info for chunk {current_chunk_id}: {e}"
        )
        return None  # Return None specifically on DB error


def get_previous_chunk_info(
    db: Engine, current_chunk_id: int
) -> Optional[Tuple[NarrativeChunk, Optional[KnownPlace]]]:
...
def process_chunk(
    ...
    if prev_chunks_info is None:
        ...
        logger.error(
            f"Could not retrieve previous chunk info for chunk {chunk.id} due to database error. Stopping batch."
        )
    ...
    if prev_chunks_info:
        ...
        for _, place in prev_chunks_info:
            if place:
                prev_place = place
                logger.info(
                    f"Most recent previous location found: '{prev_place.name}' (ID: {prev_place.id}) from chunk {prev_chunks_info[0][0].id if prev_chunks_info else 'N/A'}"
                )
                break
        else:
    ...
    hidden_locations = (
        SETTINGS.get("API Settings", {}).get("map_builder", {}).get("hidden", [])
    )
    ...
    prompt = build_location_extraction_prompt(
        chunk, known_places_formatted, prev_chunks_info=prev_chunks_info
    )
    if verbose:
        logger.debug(
            f"Prompt for chunk {chunk.id}:\n{prompt[:500]}...\n...\n{prompt[-500:]}"
        )
    ...
    try:
        ...
        llm_output = get_llm_structured_response(
            prompt, provider_instance, chunk.id, verbose, thinking_mode
        )
    except Exception as e:
    ...
    if not llm_output or not isinstance(llm_output, dict):
        logger.error(
            f"Failed to get valid structured response from LLM for chunk {chunk.id}."
        )
        ...
        while True:  # Loop until valid input or user quits
            try:
                ...
                if action_choice == "q":
                    logger.warning(
                        f"User chose to quit during manual correction for chunk {chunk.id}."
                    )
                    ...
                    if not places:
                        ...
                        visible_places_for_manual = {
                            pid: p
                            for pid, p in places.items()
                            if pid not in hidden_locations_int
                        }
                        sorted_places_manual = sorted(
                            visible_places_for_manual.values(), key=lambda p: p.id
                        )
                        if not sorted_places_manual:
                            logger.warning(
                                "All places are hidden! Showing all places instead."
                            )
                            sorted_places_manual = sorted(
                                places.values(), key=lambda p: p.id
                            )

                        print("\nLink to which existing place?")
                        for place in sorted_places_manual:
                            zone_name = (
                                zones.get(place.zone).name
                                if place.zone in zones
                                else "Unknown zone"
                            )
                            print(
                                f"ID {place.id}: {place.name} (Type: {place.type}, Zone: {zone_name})"
                            )
                        ...
                        while True:
                            try:
                                link_choice_str = input(
                                    f"Enter place ID to link to (or 'c' to cancel): "
                                )
                        ...
                        if valid_link:
                            ...
                            logger.info(
                                f"User manually linked chunk {chunk.id} to existing place '{selected_place.name}' (ID: {selected_place.id})."
                            )
                    ...
                    new_type = (
                        input("Place type [fixed_location]: ") or "fixed_location"
                    )
                    ...
                    summary_display = (
                        f", Summary: '{new_summary}'" if new_summary else ""
                    )
                    print(
                    ...
                    if confirm_add == "1":
                        ...
                        new_place_obj = add_new_place(
                            db, manual_place_data, places, zones
                        )
                        if new_place_obj:
                            place_id = new_place_obj.id
                            logger.info(
                                f"User manually added new place '{new_place_obj.name}' (ID: {place_id}) for chunk {chunk.id}."
                            )
                            # We have a valid place_id, exit manual correction loop
                            primary_location = {}  # Reset AI's primary_location
                            ...
                            logger.error(
                                f"Failed to add manually entered place '{new_name}'."
                            )
                            print(
                                "Error adding place to database. Please try again or choose another option."
                            )
                    ...
                    logger.warning(
                        f"User chose to skip chunk {chunk.id} due to LLM error."
                    )
                ...
                logger.warning(
                    "EOF detected during manual correction. Stopping processing."
                )
                return 0  # Return 0 to indicate user requested stop
        ...
        if status == "known":
            ...
            if place_id not in places:
                logger.error(
                    f"LLM provided invalid known place ID {place_id}. Triggering manual correction."
                )
                llm_output = None  # Force manual correction
                place_id = (
                    None  # Ensure place_id is None for the manual loop check below
                )
            else:
                # Place ID is valid, get the name
                place_name = places[place_id].name
                logger.info(
                    f"Identified known place '{place_name}' (ID: {place_id}) with confidence {confidence:.2f}"
                )
            # --- END EDIT ---
        elif status == "new":
            ...
            logger.error(
                f"Invalid status '{status}' in LLM output. Triggering manual correction."
            )
            llm_output = None  # Force manual correction
        ...
        if llm_output:
            ...
            if needs_review and not dry_run:
                ...
                try:
                    if (
                        status == "known"
                    ):  # Review for known (changed or first location)
                        print(f"\nREVIEW LOCATION for chunk {chunk.id}:")
                        if prev_place:
                            print(
                                f"Previous location: '{prev_place.name}' (ID: {prev_place.id})"
                            )
                        else:
                            print("Previous location: None")
                        ...
                        context_length = 200
                        context = (
                            chunk.raw_text[:context_length] + "..."
                            if len(chunk.raw_text) > context_length
                            else chunk.raw_text
                        )
                        print(f"\nContext: {context}")
                        ...
                        if prev_place:
                            print("(4) Keep previous location")
                        ...
                        while action_choice not in valid_choices:
                            action_choice = input(prompt_str).lower()

                        if action_choice == "1":
                            logger.info(
                                f"User accepted suggested location '{place_name}' (ID: {place_id})"
                            )
                        elif action_choice == "2":
                            # Code to choose different existing (simplified, combines logic from previous state)
                            if not places:
                                logger.warning("No known places to choose from.")
                                print("No known places available.")
                                place_id = None
                            else:
                                visible_places_review = {
                                    pid: p
                                    for pid, p in places.items()
                                    if pid not in hidden_locations_int
                                }
                                sorted_places_review = sorted(
                                    visible_places_review.values(), key=lambda p: p.id
                                )
                                if not sorted_places_review:
                                    sorted_places_review = sorted(
                                        places.values(), key=lambda p: p.id
                                    )  # Show all if all hidden
                                ...
                                for zone_id, zone_places in sorted(
                                    places_by_zone.items()
                                ):
                                    # Get zone name
                                    zone_name = (
                                        zones[zone_id].name
                                        if zone_id in zones
                                        else f"Unknown Zone (ID: {zone_id})"
                                    )
                                    ...
                                    # Add places in this zone
                                    for place in sorted(
                                        zone_places, key=lambda p: p.id
                                    ):
                                        print(
                                            f"├─── {place.id}: {place.name} ({place.type})"
                                        )
                                ...
                                while True:
                                    try:
                                        link_choice_str = input(
                                            "Enter place ID to use (or 'c' to cancel): "
                                        )
                                        if link_choice_str.lower() == "c":
                                            break
                                        link_choice_id = int(link_choice_str)
                                ...
                                if valid_link:
                                    selected_place = places[link_choice_id]
                                    logger.info(
                                        f"User selected alternative location '{selected_place.name}' (ID: {selected_place.id})."
                                    )
                                    place_id = selected_place.id
                                else:
                                    logger.warning(
                                        "Location selection cancelled by user. Skipping chunk."
                                    )
                                    place_id = None
                        elif action_choice == "3":
                            ...
                            if not new_name:
                                print("Place name cannot be empty.")
                                logger.warning(
                                    "Empty place name entered. Skipping chunk."
                                )
                                place_id = None
                            else:
                                print(
                                    f"Place type options: fixed_location, vehicle, other"
                                )
                                new_type = (
                                    input("Place type [fixed_location]: ")
                                    or "fixed_location"
                                )
                                if new_type not in [
                                    "fixed_location",
                                    ...
                                    print(
                                        f"Invalid type '{new_type}', using 'fixed_location'."
                                    )
                                    new_type = "fixed_location"
                                ...
                                if confirm_add == "1":
                                    ...
                                    if new_place_obj:
                                        place_id = new_place_obj.id
                                        logger.info(
                                            f"User added new place '{new_place_obj.name}' (ID: {place_id}) for chunk {chunk.id}."
                                        )
                                    else:
                                        logger.error(
                                            f"Failed to add manually entered place '{new_name}'."
                                        ...
                                        print(
                                            "Error adding place to database. Skipping chunk."
                                        )
                                        place_id = None
                                else:
                                    print("Place addition cancelled.")
                            ...
                            logger.info(
                                f"User chose to keep previous location '{prev_place.name}' (ID: {prev_place.id})"
                            )
                            ...
                            logger.warning(
                                f"User chose to quit during review for chunk {chunk.id}."
                            )
                        ...
                        zone_name_new = (
                            zones.get(zone_id_new).name
                            if zone_id_new in zones
                            else f"Unknown (ID: {zone_id_new})"
                        )
                        ...
                        if search_name_new in chunk_text_new:
                            ...
                            end_pos = min(
                                len(chunk_text_new),
                                name_pos + len(search_name_new) + 150,
                            )
                            if start_pos > 0:
                                while (
                                    start_pos > 0
                                    and chunk_text_new[start_pos] != " "
                                    and chunk_text_new[start_pos] != "\n"
                                ):
                                    start_pos -= 1
                            if end_pos < len(chunk_text_new):
                                while (
                                    end_pos < len(chunk_text_new)
                                    and chunk_text_new[end_pos] != " "
                                    and chunk_text_new[end_pos] != "\n"
                                ):
                                    end_pos += 1
                            context_new = (
                            ...
                            if heading_marker in chunk.raw_text:
                                ...
                                if end_line > heading_pos:
                                    context_new = chunk.raw_text[
                                        heading_pos:end_line
                                    ].strip()

                        print(f"\nNEW PLACE SUGGESTED for chunk {chunk.id}:")
                        print(
                            f"Name: '{place_name_new}', Type: {place_type_new}, Zone: {zone_name_new}"
                        )
                        ...
                        action_choice_new = ""
                        while action_choice_new not in ["1", "2", "3", "4", "q"]:
                            action_choice_new = input(
                                "Choose option [1-4, q]: "
                            ).lower()
                        ...
                        if action_choice_new == "1":  # Add new place
                            if zone_id_new not in zones:  # Zone Validation
                                ...
                                try:
                                    ...
                                    if zone_input_new in zones:
                                        primary_location["zone"] = zone_input_new
                                    else:
                                        logger.error(
                                            f"Invalid zone ID {zone_input_new}. Skipping add."
                                        )
                                        place_id = None
                                except ValueError:
                                    logger.error(
                                        "Invalid zone ID format. Skipping add."
                                    )
                                    place_id = None
                            # Add if zone valid
                            if (
                                "zone" in primary_location
                                and primary_location["zone"] in zones
                            ):
                                new_place_obj = add_new_place(
                                    db, primary_location, places, zones
                                )
                                if new_place_obj:
                                    place_id = new_place_obj.id
                                else:
                            ...
                            print("\nEditing place info...")
                            edit_name = (
                                input(f"Place name [{place_name_new}]: ")
                                or place_name_new
                            )
                            edit_type = (
                                input(f"Place type [{place_type_new}]: ")
                                or place_type_new
                            )
                            if edit_type not in ["fixed_location", "vehicle", "other"]:
                                edit_type = place_type_new
                            print("\nAvailable zones:")
                            for z_id, zone in sorted(zones.items()):
                                print(f"[{z_id}] {zone.name}")
                            edit_zone_id = -1
                            while True:
                                try:
                                    edit_zone_str = input(
                                        f"Zone ID [{zone_id_new}]: "
                                    ) or str(zone_id_new)
                                    edit_zone_id = int(edit_zone_str)
                                    if edit_zone_id in zones:
                                        break
                                    else:
                                        print("Invalid zone ID.")
                                except ValueError:
                            ...
                            summary_display = (
                                f", Summary: '{edit_summary}'" if edit_summary else ""
                            )
                            print(
                            ...
                            if input("Confirm [1=Yes, 0=No]: ") == "1":
                                ...
                                new_place_obj = add_new_place(
                                    db, edited_place_data, places, zones
                                )
                                if new_place_obj:
                                    place_id = new_place_obj.id
                                else:
                            ...
                            if not places:
                                ...
                                sorted_places_link = sorted(
                                    visible_places_link.values(), key=lambda p: p.id
                                )
                                if not sorted_places_link:
                                    sorted_places_link = sorted(
                                        places.values(), key=lambda p: p.id
                                    )
                                print("\nLink to which existing place?")
                                ...
                                link_id_new = -1
                                valid_link_new = False
                                while True:
                                    try:
                                        link_str_new = input(
                                            "Enter place ID to link to (or 'c' to cancel): "
                                        )
                                        if link_str_new.lower() == "c":
                                            break
                                        link_id_new = int(link_str_new)
                                        if link_id_new in places:
                                            valid_link_new = True
                                            break
                                        else:
                                            print("Invalid ID.")
                                        ...
                                        print("Invalid input.")
                                if valid_link_new:
                                    place_id = link_id_new
                                else:
                                    place_id = None  # Cancelled
                        elif action_choice_new == "4":  # Discard
                            logger.info(
                                f"User discarded new place suggestion '{place_name_new}'."
                            )
                            place_id = None
                        elif action_choice_new == "q":  # Quit
                            logger.warning(
                                f"User chose to quit during new place review for chunk {chunk.id}."
                            )
                            return 0  # User requested stop
    ...
    if place_id is not None:
        ...
        logger.warning(
            f"No valid place ID determined or selected for chunk {chunk.id}. Skipping metadata update."
        )

    # 7. Ask user whether to continue AFTER reviews, if any reviews happened (and not dry_run and not auto)
    if (
        reviews_were_presented and not auto and not dry_run
    ):  # Check if any reviews actually happened
        try:
    ...
    return (
        0 if user_requested_stop else 1
    )  # 1 = Success/Continue, 0 = Stop, -1 = Error handled earlier
...
def main():
    ...
    if args.test:
        ...
        try:
            ...
            known_places_formatted_test = format_known_places_for_prompt(
                known_places_test, zones_test
            )
        except Exception as e:
        ...
        try:
            ...
            if args.start:
                logger.info(
                    f"Using specific chunk ID {args.start} for test prompt as specified by --start"
                )
                # Get a specific chunk based on start parameter
                test_chunks = get_chunks_to_process(
                    db_engine_test, args.start, args.start, overwrite=True
                )
                if not test_chunks:
                    logger.error(
                        f"Could not find chunk with ID {args.start}. Please check the ID and try again."
                    )
                    return 1
            else:
                ...
                test_chunks = get_chunks_to_process(
                    db_engine_test, overwrite=args.overwrite
                )
                if not test_chunks:
            ...
            if prev_chunk_info:
                ...
                if prev_place:
                    logger.info(
                        f"Previous chunk {prev_chunk.id} had location: '{prev_place.name}' (ID: {prev_place.id})"
                    )
                else:
            ...
            test_prompt = build_location_extraction_prompt(
                ...
                prev_chunks_info=get_previous_chunks_info(
                    db_engine_test, test_chunk.id
                ),
            )
    ...
    try:
        if not check_database_schema(db_engine):
            logger.error(
                "Database schema validation failed. Please verify your database structure."
            )
            return 1
    except Exception as e:
    ...
    if not zones:
        logger.error(
            "No zones loaded from the database. Cannot proceed without zone data."
        )
        return 1

    if not known_places:
        logger.warning(
            "No known places loaded from the database. Processing will assume all locations are new initially."
        )
    ...
    try:
        provider_kwargs = {
            ...
            "model": args.model
            or DEFAULT_MODEL_FOR_SCRIPT,  # Ensure script default is used if arg not provided
            "temperature": (
                args.temperature if args.temperature is not None else 0.1
            ),  # Default temp if not provided
    ...
    for i in range(0, total_chunks, batch_size):
        ...
        logger.info(
            f"--- Starting Batch {i // batch_size + 1} / {(total_chunks + batch_size - 1) // batch_size} (Chunks {batch[0].id} to {batch[-1].id}) ---"
        )

        for chunk in batch:
            processed_count += 1
            logger.info(
                f"Processing chunk {chunk.id} ({processed_count}/{total_chunks})..."
            )
            ...
            try:
                ...
                if should_continue == 1:
                    ...
                    # User requested stop
                    logger.info(
                        f"Stopping processing after chunk {chunk.id} as requested by user."
                    )
                    success_count += (
                        1  # The chunk was successfully processed up to the stop point
                    )
                    ...
                    # Error case
                    logger.error(
                        f"Processing failed for chunk {chunk.id}, counting as an error."
                    )
                    error_count += 1
                ...
                logger.warning(
                    f"Stopping current batch due to unhandled error in chunk {chunk.id}."
                )
        ...
        if (
            ...
            if not args.auto:  # Only prompt between batches if NOT in auto mode
                ...
                if batch_delay > 0:
                    logger.info(
                        f"Auto mode: Waiting {batch_delay}s before next batch..."
                    )
