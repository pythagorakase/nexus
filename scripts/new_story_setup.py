...
def create_assets_tables(dbname: Optional[str] = None) -> None:
    ...
    LOG.info("Ensured assets tables exist in %s", dbname or os.environ.get("PGDATABASE", "NEXUS"))
...
def create_slot_schema_only(slot: int, source_db: Optional[str], force: bool = False) -> None:
    """Create a per-slot database with schema only (no data)."""
    if slot < 1 or slot > 5:
        raise ValueError("Slot must be between 1 and 5 (inclusive)")
    source_db = source_db or os.environ.get("PGDATABASE", "NEXUS")
