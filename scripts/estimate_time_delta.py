...
DEFAULT_TPM_LIMITS = {"openai": 30000, "anthropic": 20000}
...
class LLMProvider:
    ...
    def from_provider_name(cls, provider_name: str, **kwargs) -> "LLMProvider":
        ...
        """
        logger.info(
            f"DEBUG: LLMProvider.from_provider_name called with provider_name={provider_name}"
        )
        ...
        logger.info(
            f"DEBUG: Creating provider instance for class {provider_class.__name__}"
        )
        try:
...
class OpenAIProvider(LLMProvider):
    ...
    def _get_reasoned_completion(self, prompt: str) -> LLMResponse:
        ...
        logger.info(
            f"Using OpenAI responses API with model {self.model}, reasoning effort: {self.reasoning_effort}, and structured output"
        )
            ...
            if hasattr(response.reasoning, "summary"):
                try:
                    if isinstance(response.reasoning.summary, list):
                        ...
                        logger.info(
                            f"Summary (format: {type(response.reasoning.summary)}): {response.reasoning.summary}"
                        )
                except Exception as e:
                ...
                logger.info(
                    f"Reasoning attributes: {', '.join(dir(response.reasoning))}"
                )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Estimate time delta for narrative chunks"
    )
    ...
    chunk_selection.add_argument(
        "--all", action="store_true", help="Process all chunks needing time_delta"
    )

    # Other arguments
    parser.add_argument(
        "--end", type=int, help="Ending chunk id number (required with --start)"
    )
    parser.add_argument(
        "--test", action="store_true", help="Test mode, print results only"
    )
    parser.add_argument(
        "--write", action="store_true", help="Write results to database"
    )
    parser.add_argument(
        "--primary-only",
        action="store_true",
    ...
    parser.add_argument(
        "--model", type=str, help="Model name (defaults to provider's default model)"
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    if args.primary_only and args.include_all_world_layers:
        parser.error(
            "Cannot specify both --primary-only and --include-all-world-layers"
        )
...
def get_chunk_by_id(db: Engine, chunk_id: int) -> Optional[NarrativeChunk]:
    ...
    try:
        with db.connect() as conn:
            ...
            if result:
                return NarrativeChunk(id=result[0], raw_text=result[1])
            return None
    except Exception as e:
...
def get_chunks_in_range(db: Engine, start_id: int, end_id: int) -> List[NarrativeChunk]:
    ...
    try:
        with db.connect() as conn:
            ...
            return [NarrativeChunk(id=row[0], raw_text=row[1]) for row in results]
    except Exception as e:
        logger.error(f"Error getting chunks in range {start_id}-{end_id}: {str(e)}")
        return []


def get_all_chunks_needing_time_delta(
    db: Engine, primary_only: bool = False
) -> List[NarrativeChunk]:
    """
...
def get_chunk_world_layer(db: Engine, chunk_id: int) -> str:
    ...
    try:
        ...
        logger.warning(
            f"Error fetching chunk world_layer for chunk {chunk_id}: {str(e)}"
        )
...
def get_context_chunks(
    db: Engine, target_id: int
) -> Tuple[Optional[NarrativeChunk], Optional[NarrativeChunk]]:
...
def get_extended_context_chunks(
    db: Engine, target_id: int
) -> Tuple[List[NarrativeChunk], List[NarrativeChunk]]:
    """
    ...
    try:
        ...
        if current_id >= 1:
            ...
            if prev_chunk:
                ...
                if prev_world_layer != "primary":
                    ...
                    while scan_id >= 1 and (
                        not meta_start_found or not primary_last_found
                    ):
                        ...
                        if in_meta_sequence and scan_world_layer != "primary":
                            ...
                            if scan_id == current_id - 1:
                                ...
                                if prev_scan_chunk:
                                    prev_scan_world_layer = get_chunk_world_layer(
                                        db, prev_scan_chunk.id
                                    )
        ...
        logger.error(
            f"Error getting extended context chunks for id {target_id}: {str(e)}"
        )

    return prev_chunks, next_chunks


def build_time_estimation_prompt(
    chunk: NarrativeChunk,
    prev_chunks: List[NarrativeChunk] = None,
    next_chunks: List[NarrativeChunk] = None,
    prev_chunk_time_delta: Optional[str] = None,
) -> str:
    """
    ...
    """
    prompt = load(
        PromptId.OPERATORS_ESTIMATE_TIME_DELTA_SYSTEM,
        CHUNK_ID=f"{chunk.id}",
    ...
    if prev_chunks and len(prev_chunks) > 0:
        ...
        for i, prev_chunk in enumerate(prev_chunks):
            ...
            if i == len(prev_chunks) - 1 and prev_chunk_time_delta:
                time_delta_info = (
                    f" [TIME ALREADY ACCOUNTED FOR: {prev_chunk_time_delta}]"
                )
...
"""
    ...
    # Add instructions
    prompt += load(PromptId.OPERATORS_ESTIMATE_TIME_DELTA_USER)
...
"""
    ...
    try:
        ...
        logger.info(
            f"DEBUG: Setting up provider kwargs with model={model}, temperature={temperature}"
        )
        provider_kwargs = {
            "model": model,
            "temperature": temperature,
        ...
        if provider.lower() == "openai":
            logger.info(
                f"DEBUG: Adding reasoning_effort={reasoning_effort} for OpenAI provider"
            )
        ...
        llm = LLMProvider.from_provider_name(provider_name=provider, **provider_kwargs)
        ...
        logger.info(
            f"DEBUG: TPM check complete: within_limit={within_limit}, tokens={total_tokens}"
        )
        ...
        if not within_limit:
            ...
            logger.error(
                f"Estimated tokens: {total_tokens} (input: {input_tokens}, output: ~{estimated_output_tokens})"
            )
            logger.error(f"Try processing a smaller chunk or use a different provider")
            raise ValueError(
                f"TPM limit exceeded for {provider}. Too many tokens in a single request."
            )
        ...
        # Log token usage
        logger.info(
            f"API success: {response.input_tokens} input tokens, {response.output_tokens} output tokens"
        )
        ...
        if provider.lower() == "openai":
            if "insufficient_quota" in str(e) or "exceeded your current quota" in str(
                e
            ):
                logger.error(
                ...
                logger.error(
                    "OpenAI API rate limit reached. Try again later or reduce request frequency."
                )
        elif provider.lower() == "anthropic":
            if "rate_limit" in str(e) or "rate limit" in str(e).lower():
                logger.error(
                    "Anthropic API rate limit reached. Try again later or reduce request frequency."
                )
...
def parse_time_delta(
    response_input: Union[str, LLMResponse], test_mode: bool = False
) -> timedelta:
    ...
    if (
        isinstance(response_input, LLMResponse)
        and response_input.structured_data is not None
    ):
        try:
            ...
            if "total_hours" in data and "total_minutes" in data:
                ...
                if "activities" in data:
                    ...
                    for activity in activities:
                        logger.info(
                            f"  - {activity.get('name')}: {activity.get('minutes')} minutes"
                        )

                return timedelta(hours=total_hours, minutes=total_minutes)
            else:
                logger.warning(
                    "Structured data found but missing 'total_hours' or 'total_minutes'"
                )
        except Exception as e:
    ...
    response_text = (
        ...
        if isinstance(response_input, LLMResponse)
        else response_input
    )
    ...
    try:
        ...
        for potential_json in json_matches:
            try:
                ...
                if "total_hours" in data and "total_minutes" in data:
                    ...
                    logger.info(
                        f"Parsed JSON from response text: {total_hours}h {total_minutes}m"
                    )
        ...
        try:
            ...
            if "total_hours" in data and "total_minutes" in data:
                ...
                total_minutes = int(data.get("total_minutes", 0))
                logger.info(
                    f"Parsed entire response as JSON: {total_hours}h {total_minutes}m"
                )
                return timedelta(hours=total_hours, minutes=total_minutes)
        except Exception as e:
        ...
        if json_matches:
            ...
            logger.info(
                f"Successfully parsed JSON from code block: {total_hours} hours, {total_minutes} minutes"
            )
...
def update_chunk_metadata(
    db: Engine, chunk_id: int, time_delta: timedelta, test_mode: bool = False
) -> bool:
    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                "SELECT 1 FROM chunk_metadata WHERE chunk_id = %s", (chunk_id,)
            )
            ...
            cursor.execute(
                "SELECT time_delta FROM chunk_metadata WHERE chunk_id = %s", (chunk_id,)
            )
            ...
            if result:
                ...
                logger.warning(
                    "Update verification failed - could not find record after update"
                )

            # Commit the transaction
            conn.commit()
            logger.info(
                f"Successfully committed transaction for chunk {chunk_id} with time_delta '{formatted_time}'"
            )
            return True
...
def process_chunk(
    ...
    if time_delta_cache is None:
        ...
        logger.info(
            f"DEBUG: Received time_delta_cache with {len(time_delta_cache)} entries"
        )
    ...
    logger.info(
        f"Processing chunk #{chunk.id} (id: {chunk.id}, world_layer: {world_layer})"
    )
    ...
    if world_layer != "primary":
        ...
        logger.info(
            f"Chunk #{chunk.id} has world_layer '{world_layer}' - setting time_delta to 0 minutes"
        )
    ...
    logger.info(
        f"DEBUG: Got {len(prev_chunks)} previous chunks and {len(next_chunks)} next chunks"
    )
    ...
    if prev_chunks and len(prev_chunks) > 0:
        logger.info(
            f"DEBUG: Getting time_delta for previous chunk #{prev_chunks[-1].id}"
        )
        ...
        if immediate_prev_chunk.id in time_delta_cache:
            ...
            logger.info(
                f"Using cached time_delta for chunk #{immediate_prev_chunk.id}: {prev_chunk_time_delta}"
            )
        else:
            # Not in cache, check database
            logger.info(
                f"DEBUG: Looking up prev chunk #{immediate_prev_chunk.id} in database"
            )
            prev_chunk_time_delta = get_chunk_time_delta(db, immediate_prev_chunk.id)
            if prev_chunk_time_delta:
                logger.info(
                    f"Found time_delta in database for chunk #{immediate_prev_chunk.id}: {prev_chunk_time_delta}"
                )

    # Build prompt with context chunks
    logger.info(
        f"DEBUG: Building prompt for chunk #{chunk.id} with prev_chunk_time_delta={prev_chunk_time_delta}"
    )
...
def main() -> int:
    ...
    if args.verbose:
        logger.info(
            "Verbose mode enabled - will print detailed prompts and context information"
        )
    ...
    if args.all:
        ...
        logger.info(
            f"Retrieving all chunks needing time_delta (primary_only={primary_only})"
        )
    ...
    stats = {
        ...
        "reasoning_effort": (
            args.reasoning_effort if args.provider.lower() == "openai" else None
        ),
        "primary_chunks": 0,
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        logger.info(
            f"Starting batch {i//batch_size + 1} of {(len(chunks) + batch_size - 1) // batch_size}"
        )
            ...
            try:
                ...
                if world_layer != "primary" and args.verbose:
                    try:
                        ...
                        prev_chunks, next_chunks = get_extended_context_chunks(
                            db, chunk.id
                        )

                        print("\n" + "=" * 80)
                        print(
                            f"VERBOSE: Processing chunk #{chunk.id} (world_layer: {world_layer})"
                        )
                        print(
                            f"VERBOSE: Non-primary world layer chunk - will automatically get 0 minutes"
                        )
                        print(
                            f"VERBOSE: Context includes {len(prev_chunks)} previous chunks and {len(next_chunks)} next chunks"
                        )
                        ...
                        if prev_chunks:
                            ...
                            for i, prev in enumerate(prev_chunks):
                                ...
                                print(
                                    f"  - Chunk #{prev.id} (world_layer: {prev_layer}) - {role}"
                                )
                        print("=" * 80 + "\n")
                    except Exception as e:
                        logger.warning(
                            f"Error in verbose mode for non-primary chunk #{chunk.id}: {str(e)}"
                        )
                ...
                if args.test and world_layer == "primary":
                    try:
                        ...
                        if args.verbose:
                            ...
                            prompt = build_time_estimation_prompt(
                                ...
                                prev_chunks=prev_chunks,
                                next_chunks=next_chunks,
                                prev_chunk_time_delta=(
                                    get_chunk_time_delta(db, prev_chunks[-1].id)
                                    if prev_chunks and len(prev_chunks) > 0
                                    else None
                                ),
                            )
                        ...
                        logger.warning(
                            f"Error in verbose mode for chunk #{chunk.id}: {str(e)}"
                        )
                    ...
                    formatted_time = format_timedelta(time_delta)
                    logger.info(
                        f"TEST MODE: Using default time delta of {formatted_time} for primary chunk #{chunk.id}"
                    )
                ...
                if args.test:
                    ...
                    logger.error(
                        f"TEST MODE Error processing chunk #{chunk.id} (world_layer: {current_world_layer}): {str(e)}"
                    )
        ...
        # Print batch summary
        logger.info(
            f"Batch complete. Progress: {stats['processed_chunks']}/{stats['total_chunks']} chunks"
        )
        logger.info(f"API calls: {stats['api_calls']} (errors: {stats['api_errors']})")
        logger.info(
            f"Primary chunks: {stats['primary_chunks']}, Non-primary: {stats['non_primary_chunks']}"
        )
        ...
        if (
            stats["api_calls"] > 0
            and stats["api_errors"] / max(1, stats["api_calls"]) > 0.3
        ):
            rate_limited = True
        ...
        if i + batch_size < len(chunks) and not args.test:
            if rate_limited:
                ...
                logger.info(
                    f"Rate limit detected. Waiting {wait_time} seconds before continuing..."
                )

                if not args.auto:
                    # In manual mode, ask if user wants to wait or stop
                    continue_val = input(
                        f"Continue after waiting {wait_time} seconds? [Y/n]: "
                    )
                ...
                logger.info(
                    f"Auto mode: Waiting {wait_time} seconds before next batch..."
                )
                time.sleep(wait_time)
            else:
    ...
    logger.info(
        f"Processed {stats['processed_chunks']}/{stats['total_chunks']} chunks successfully"
    )
