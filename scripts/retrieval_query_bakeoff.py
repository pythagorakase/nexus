...
This is a diagnostic harness, not runtime narrative behavior. It compares three
...
* ``local_llm``: queries generated by the local LLM from warm context analysis.
...
def _build_analysis_from_refs(sample: TurnSample) -> dict[str, Any]:
    return {
        "characters": sample.source_refs.characters,
        "locations": sample.source_refs.places,
        "context_type": "unknown",
        "entities_for_retrieval": (
            sample.source_refs.characters[:6]
            + sample.source_refs.places[:4]
            + sample.source_refs.factions[:4]
        ),
    }


def _warm_slice_for_sample(sample: TurnSample) -> list[dict[str, Any]]:
    return [
        {
            "id": sample.source_chunk_id,
            "chunk_id": sample.source_chunk_id,
...
def generate_local_queries(
    ...
    *,
    ...
    """Generate or load local-LLM queries for one turn sample."""

    cache_key = f"{sample.key}:local_llm"
    cached = cache.get(cache_key)
    if cached:
        return dedupe_queries(cached)

    analysis = llm_manager.analyze_narrative_context(
        _warm_slice_for_sample(sample), sample.choice_text
    )
    queries = llm_manager.generate_retrieval_queries(analysis, sample.choice_text)
    ...
    *,
    ...
    if local_queries:
        query_sets.append(
            QuerySet(
                name="local_llm",
                description="Local-LLM generated retrieval queries",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Generate local LLM queries instead of using cached ones only",
    )
    parser.add_argument(
        "--cache",
        type=Path,
...
def _init_local_llm_manager() -> Any:
...
def main() -> None:
    ...
    for index, sample in enumerate(samples, start=1):
        ...
        if local_llm is not None:
            local_queries = generate_local_queries(
                sample, cache=cache, llm_manager=local_llm
            )
        else:
