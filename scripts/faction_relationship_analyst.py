...
def call_openai_api_faction_to_faction(
    messages: list, test_mode: bool = False
...
def save_faction_to_faction_relationship(
    ...
    if dry_run:
        logger.info(
            f"[DRY RUN] Would save faction relationship {faction1_id} <-> {faction2_id}"
        )
    ...
    try:
        ...
        if exists:
            ...
            logger.info(
                f"Updated existing faction relationship {faction1_id} <-> {faction2_id}"
            )
        else:
            ...
            logger.info(
                f"Inserted new faction relationship {faction1_id} <-> {faction2_id}"
            )
...
def save_faction_to_character_relationship(
    ...
    if dry_run:
        logger.info(
            f"[DRY RUN] Would save faction-character relationship: "
    ...
    try:
        ...
        if exists:
            ...
            logger.info(
                f"Updated existing faction-character relationship: "
                f"Faction {faction_id} -> Character {character_id}"
            )
        else:
            # Insert new relationship
            ...
            logger.info(
                f"Inserted new faction-character relationship: "
                f"Faction {faction_id} -> Character {character_id}"
            )
...
def main():
    ...
    if args.character is not None:
        ...
        logger.info(
            f"Processing faction-to-character relationship: "
        ...
        if len(faction_ids) != 2:
            logger.error(
                "Exactly 2 faction IDs required for faction-to-faction relationships"
            )
            sys.exit(1)
        ...
        logger.info(
            f"Processing faction-to-faction relationship: "
