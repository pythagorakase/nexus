...
- Uses chat completions API directly to potentially avoid token limit issues

Implementation Note:
This script was modified to use the OpenAI chat completions API directly rather than the
provider.get_completion() method. This change was implemented to test if it resolves token limit
issues experienced with very large prompts. The theory is that the chat completions API might
have different token limit behavior than the responses API for large context windows.
...
# Import OpenAI SDK directly for chat completions
try:
    import openai
except ImportError:
    print("Error: OpenAI package is required. Please install with: pip install openai")
    sys.exit(1)
...
from scripts.api_openai import (
    OpenAIProvider,
    LLMResponse,  # Import LLMResponse for compatibility
