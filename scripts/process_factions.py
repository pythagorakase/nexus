...
class FactionReference(BaseModel):
    ...
    context: str = Field(
        description="Brief excerpt showing the faction reference in context"
    )
...
def save_faction_references(
    conn, chunk_id: int, references: List[FactionReference], dry_run: bool = False
):
    """Save faction references to the database"""
    if dry_run:
        logger.info(
            f"[DRY RUN] Would save {len(references)} faction references for chunk {chunk_id}"
        )
        for ref in references:
            logger.info(
                f"  - Faction {ref.faction_id} ({ref.faction_name}), confidence: {ref.confidence}"
            )
        return
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(
                f"Saved {len(references)} faction references for chunk {chunk_id}"
            )
    except Exception as e:
        conn.rollback()
...
def create_system_prompt(faction_roster: List[Dict[str, Any]]) -> str:
    ...
    for faction in faction_roster:
        ...
        # Otherwise fall back to ideology if available
            ...
            ideology = (
                faction["ideology"][:100] + "..."
                if len(faction["ideology"]) > 100
                else faction["ideology"]
            )
    ...
    system_prompt = load(
        PromptId.OPERATORS_PROCESS_FACTIONS, ROSTER_TEXT=f"{roster_text}"
    )
...
def create_user_message(
    target_chunk: Dict, leading_chunk: Optional[Dict], trailing_chunk: Optional[Dict]
) -> str:
        ...
        try:
            ...
            if result:
                ...
                if result.faction_references:
                    logger.info(
                        f"Found {len(result.faction_references)} faction references:"
                    )
                    for ref in result.faction_references:
                        logger.info(
                            f"  - {ref.faction_name} (ID: {ref.faction_id}, confidence: {ref.confidence})"
                        )
                else:
...
def main():
    ...
    # Required arguments
    parser.add_argument("--start", type=int, required=True, help="Starting chunk ID")

    # Optional arguments
    parser.add_argument(
        "--end", type=int, help="Ending chunk ID (defaults to start if not specified)"
    )
    parser.add_argument(
        "--model", default="o3", help="OpenAI model to use (default: o3)"
    )
    parser.add_argument(
        "--effort",
