...
def migrate_small_embeddings():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            if count > 0:
                logger.info(
                    f"Found {count} small embeddings already in chunk_embeddings"
                )
                return
            ...
            for i in range(0, len(small_embeddings), batch_size):
                ...
                for chunk_id, model, embedding, created_at in batch:
                    try:
                        ...
                        if processed % 100 == 0:
                            logger.info(
                                f"Processed {processed}/{len(small_embeddings)} embeddings"
                            )

                    except Exception as e:
                        logger.error(
                            f"Error migrating embedding for chunk {chunk_id}: {e}"
                        )
            ...
            logger.info(
                f"Successfully migrated {processed} out of {len(small_embeddings)} embeddings"
            )
...
def verify_migration():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            logger.info(
                f"Verification: Found {count_384} embeddings with dimensions=384 in main table"
            )
