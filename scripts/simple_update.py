...
def find_chunk_id(
    engine: Engine, season: int, episode: int, scene: int
) -> Optional[int]:
    """
...
def find_insertion_point(engine: Engine, season: int, episode: int, scene: int) -> int:
    ...
    with engine.connect() as conn:
        ...
        if episode_exists:
            ...
            next_episode_chunk = conn.execute(
                text(
                    """
                SELECT MIN(nc.id)
                FROM narrative_chunks nc
            ...
            prev_episode_chunk = conn.execute(
                text(
                    """
                SELECT MAX(nc.id)
                FROM narrative_chunks nc
    ...
    with engine.connect() as conn:
        ...
        if seq_exists:
            ...
            logger.warning(
                f"Using calculated ID {next_id} because narrative_chunks_id_seq does not exist"
            )
...
def update_chunk(engine: Engine, chunk_id: int, raw_text: str, dry_run: bool) -> bool:
    ...
    if dry_run:
        logger.info(
            f"DRY RUN: Would update chunk {chunk_id} with {len(raw_text)} characters"
        )
        return True
...
def ensure_all_chunks_have_metadata(engine: Engine, dry_run: bool = False) -> bool:
    ...
    try:
        with engine.begin() as conn:
            ...
            chunks_without_metadata = conn.execute(
                text(
                    """
            ...
            logger.info(
                f"Found {len(chunks_without_metadata)} chunks without metadata entries"
            )
            ...
            for chunk_id, raw_text in chunks_without_metadata:
                ...
                for line in raw_text.split("\n"):
                    scene_break_match = re.match(
                        r"<!--\s*SCENE BREAK:\s*(S(\d+)E(\d+)_(\d+)).*-->", line
                    )
                ...
                if not scene_tag:
                    logger.error(
                        f"Could not extract metadata from chunk {chunk_id}, raw text: {raw_text[:100]}..."
                    )
                    continue

                logger.info(
                    f"Extracted metadata for chunk {chunk_id}: {scene_tag} (S{season}E{episode}, scene {scene})"
                )
            ...
            if remaining_chunks > 0:
                logger.warning(
                    f"There are still {remaining_chunks} chunks without metadata entries"
                )
                return False
            else:
...
def resequence_all_chunks(
    engine: Engine, dry_run: bool = False, start_id: int = 1, increment: int = 1
) -> bool:
    """
    ...
    if dry_run:
        logger.info(
            f"DRY RUN: Would resequence all chunks starting at ID {start_id} with increment {increment}"
        )
        return True
    ...
    try:
        with engine.begin() as conn:
            ...
            if updated_count != total_chunks:
                logger.error(
                    f"Updated {updated_count} rows but expected {total_chunks}"
                )
                return False
            ...
            if seq_exists:
                ...
                logger.warning(
                    "narrative_chunks_id_seq does not exist, skipping sequence update"
                )
            ...
            if fk_constraints:
                for constraint in fk_constraints:
                    ...
                    logger.warning(
                        f"Found FK constraint from {table}.{column} to chunk_metadata.id with update rule {update_rule}"
                    )

                    if update_rule != "CASCADE":
                        logger.error(
                            f"Cannot safely resequence chunk_metadata IDs because {table}.{column} has a non-CASCADE update rule: {update_rule}"
                        )
                        logger.error(
                ...
                logger.info(
                    "All foreign key constraints have ON UPDATE CASCADE, proceeding with resequencing"
                )
            ...
            highest_meta_id = conn.execute(
                text("SELECT MAX(id) FROM chunk_metadata")
            ).scalar()
            ...
            if meta_seq_exists:
                ...
                logger.warning(
                    "chunk_metadata_id_seq does not exist, skipping sequence update"
                )
...
def reorganize_chunk_ids(
    engine: Engine, insertion_id: int, needed_gap: int = 1
) -> bool:
    """
    ...
    try:
        with engine.begin() as conn:
            ...
            """
                ...
                {"start_id": insertion_id, "end_id": insertion_id + gap - 1},
            ).scalar()
...
def create_chunk(
    ...
    try:
        ...
        if not reorganize_chunk_ids(engine, insertion_id, 1):
            ...
            with engine.connect() as conn:
                insertion_id = conn.execute(
                    text("SELECT nextval('narrative_chunks_id_seq')")
                ).scalar()
                logger.info(
                    f"Using sequence value {insertion_id} for {tag} after reorganization failed"
                )
...
def process_file(
    engine: Engine, file_path: Path, dry_run: bool, verbose: bool = False
) -> Tuple[int, int, int]:
    """
    ...
    if verbose:
        ...
        logger.info(
            f"Found {len(chunks)} chunks with tags from {chunk_tags[0]} to {chunk_tags[-1]}"
        )
    ...
    for tag, data in chunks.items():
        ...
        if verbose:
            logger.info(
                f"Processing chunk: {tag} (Season {season}, Episode {episode}, Scene {scene})"
            )
            if len(text) > 100:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Simple script to update raw text in narrative chunks"
    )
    ...
    for file_path in all_files:
        ...
        try:
            updated, created, errors = process_file(
                engine, Path(file_path), args.dry_run, args.verbose
            )
    ...
    if args.fix_metadata:
        ...
        if missing_count == 0:
            ...
            if ensure_all_chunks_have_metadata(engine, args.dry_run):
                ...
                if not args.dry_run:
                    print(
                        "Some chunks still don't have metadata entries. See log for details."
                    )
                    total_errors += 1
                else:
        ...
        if not args.dry_run:
            with engine.connect() as conn:
                ...
                meta_count = conn.execute(
                    text("SELECT COUNT(*) FROM chunk_metadata")
                ).scalar()
