...
def get_memnon_settings_summary(memnon_settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    summary = {"models": {}, "retrieval": {}, "hybrid_search": {}, "query": {}}
...
def run_golden_queries():
    ...
    configuration = load_settings_as_dict()
    ...
    if args.hybrid is not None:
        ...
        logger.info(
            f"Hybrid search {'enabled' if args.hybrid else 'disabled'} by command line flag"
        )
    ...
    for i, query_info in enumerate(all_queries):
        ...
        try:
            ...
            query_result = memnon.query_memory(
                query=query_text, query_type=query_type, k=args.k
            )
