...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("query_narratives.log"), logging.StreamHandler()],
)
...
def main():
    ...
    memnon = MEMNON(
        interface=ConsoleInterface(), agent_state=None, user=None, db_url=args.db_url
    )
    ...
    if args.format == "json":
        ...
        for i, result in enumerate(results):
            ...
            print(
                f"Season: {metadata.get('season', 'Unknown')}, Episode: {metadata.get('episode', 'Unknown')}"
            )
