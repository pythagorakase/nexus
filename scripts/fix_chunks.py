    ...
    try:
        ...
        for ep in episodes:
            ...
            if match:
                ...
                print(
                    f"S{season:02d}E{episode:02d}: Chunks {start_chunk}-{end_chunk-1}"
                )
                ...
                if incorrect_chunks:
                    print(
                        f"  Found {len(incorrect_chunks)} incorrectly assigned chunks in S{season:02d}E{episode:02d} range:"
                    )
                    for chunk in incorrect_chunks:
                        print(
                            f"  - Chunk {chunk['id']} assigned to S{chunk['season']:02d}E{chunk['episode']:02d}, should be S{season:02d}E{episode:02d}"
                        )
                        ...
                        print(
                            f"    - Updated chunk {chunk['id']} to S{season:02d}E{episode:02d}"
                        )
