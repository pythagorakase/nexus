...
class CharacterExpansion(BaseModel):
    ...
    background: str = Field(description="Character's history and backstory")
...
def get_character(
    engine: sa.engine.Engine, character_id: int
    ...
    with engine.connect() as conn:
        ...
        if target_character_row:
            ...
            logger.info(
                f"Retrieved character ID: {target_character['id']}, Name: {target_character['name']}"
            )
        else:
...
def get_character_episode_spans(
    engine: sa.engine.Engine, character_id: int
) -> List[Dict[str, Any]]:
    """
    ...
    for ep in episodes:
        ...
        with engine.connect() as conn:
            result = conn.execute(
                text(span_query), {"season": ep["season"], "episode": ep["episode"]}
            )
    ...
    logger.info(
        f"Found {len(episode_spans)} episode spans for character {character_id}"
    )
    for span in episode_spans:
        # Convert to int if these are strings from the database
        season = (
            int(span["season"])
            if isinstance(span["season"], (str, float))
            else span["season"]
        )
        episode = (
            int(span["episode"])
            if isinstance(span["episode"], (str, float))
            else span["episode"]
        )
        logger.info(
            f"S{season:02d}E{episode:02d}: chunks {span['start_chunk']}-{span['end_chunk']}"
        )

    return episode_spans


def get_narrative_chunks(
    ...
    if chunk_ids and chunk_ids.lower() == "auto" and character_id:
        logger.info(
            f"Using auto mode to get episode spans for character {character_id}"
        )
...
def update_character_expansion(
    engine: sa.engine.Engine, character_expansion: CharacterExpansion
) -> bool:
    """
    ...
    if character_expansion.additional_aliases:
        ...
        try:
            with engine.connect() as conn:
                result = conn.execute(
                    text(alias_query), {"id": character_expansion.character_id}
                )
                row = result.fetchone()
                if row and row[0]:
        ...
        for alias in character_expansion.additional_aliases:
            if alias not in existing_aliases:
                ...
                try:
                    with engine.connect() as conn:
                        conn.execute(
                            text(insert_alias_query),
                            {
                                "character_id": character_expansion.character_id,
                                "alias": alias,
                            },
                        )
                        conn.commit()
    ...
    extra_data_dict = (
        character_expansion.extra_data.model_dump(exclude_none=True)
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(
                text(query),
                {
                    ...
                    "emotional_state": (
                        character_expansion.emotional_state[:500]
                        if character_expansion.emotional_state
                        else None
                    ),
                    "current_activity": (
                        character_expansion.current_activity[:500]
                        if character_expansion.current_activity
                        else None
                    ),
                    "current_location": (
                        character_expansion.current_location[:500]
                        if character_expansion.current_location
                        else None
                    ),
        ...
        logger.info(
            f"Updated expansion for character ID {character_expansion.character_id}"
        )
        return True
    except Exception as e:
        logger.error(
            f"Failed to update expansion for character {character_expansion.character_id}: {e}"
        )
        return False
...
def prepare_target_character_section(target_character: Dict[str, Any]) -> str:
    ...
    target_section += (
        f"### Character ID: {target_character['id']} ({target_character['name']})\n\n"
    )
...
def prepare_character_from_json(
    context_data: Dict[str, Any], target_character_id: int
) -> str:
    """
    ...
    if "characters" in context_data and char_id_str in context_data["characters"]:
        ...
        logger.warning(
            f"Character ID {target_character_id} not found in manual context file"
        )
        target_section += (
            f"Character ID {target_character_id} not found in context file.\n\n"
        )
...
def prepare_summaries_context(
    season_summaries: List[Dict[str, Any]], episode_summaries: List[Dict[str, Any]]
) -> str:
    """
    ...
    for season in sorted(season_summaries, key=lambda x: x["id"]):
        ...
        season_num = (
            int(season_id) if isinstance(season_id, (str, float)) else season_id
        )
        ...
        if season_id in episodes_by_season:
            for episode in sorted(
                episodes_by_season[season_id], key=lambda x: x["episode"]
            ):
                # Convert to int if string from database
                season_num = (
                    int(season_id) if isinstance(season_id, (str, float)) else season_id
                ...
                episode_num = (
                    int(episode["episode"])
                    if isinstance(episode["episode"], (str, float))
...
def prepare_chunks_context(chunks: List[Dict[str, Any]]) -> str:
    ...
    for chunk in chunks:
        ...
        if season_num and episode_num:
            ...
            context += (
                f"### Chunk ID: {chunk['id']} (S{season_num:02d}E{episode_num:02d})\n\n"
            )
        else:
...
def load_manual_context(context_file: str) -> Dict[str, Any]:
    ...
    if os.path.isabs(context_file):
        ...
        context_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), context_file
        )
    ...
    try:
        ...
        for section in required_sections:
            if section not in context_data:
                logger.warning(
                    f"Manual context file is missing required section: {section}"
                )
...
def build_prompt(
    prompt_data: Dict[str, Any], target_characters: str, context: str
) -> str:
    """
...
def save_expansion_to_file(
    character_expansion: CharacterExpansion, filename: Optional[str] = None
) -> str:
    """
...
def load_expansion_from_file(filename: str) -> CharacterExpansion:
    ...
    try:
        ...
        if (
            ...
            and len(data["characters"]) > 0
        ):
            # Extract the first character from the old format
            character_data = data["characters"][0]
            logger.info(
                f"Converting legacy format from {filename} (multiple characters) to single character"
            )
        else:
        ...
        logger.info(
            f"Loaded character expansion from {filename} for character ID {character_expansion.character_id}"
        )
...
def process_character_expansion(
    character_expansion: CharacterExpansion,
    ...
    has_existing_data = (
        (character_data.get("appearance") and character_data["appearance"].strip())
        or (
            character_data.get("background")
            and character_data["background"].strip()
            and character_data["background"] != "unknown"
        )
        or (character_data.get("personality") and character_data["personality"].strip())
        or (
            character_data.get("emotional_state")
            and character_data["emotional_state"].strip()
        )
        or (
            character_data.get("current_activity")
            and character_data["current_activity"].strip()
        )
        or (
            character_data.get("current_location")
            and character_data["current_location"].strip()
        )
        or (character_data.get("extra_data") and character_data["extra_data"])
    )
    ...
    if character_expansion.additional_aliases:
        logger.info(
            f"Additional Aliases: {', '.join(character_expansion.additional_aliases)}"
        )
    ...
    if has_existing_data and not force and not dry_run:
        ...
        if character_expansion.extra_data:
            extra_data_dict = character_expansion.extra_data.model_dump(
                exclude_none=True
            )
            if extra_data_dict:
                print("Extra Data:")
        ...
        response = (
            input(f"Overwrite existing data for {char_name}? (y/n): ").strip().lower()
        )
        if response != "y":
    ...
    if not dry_run:
        ...
        if success:
            logger.info(
                f"Successfully updated expansion for character {char_id} ({char_name})"
            )
            return True
        else:
            logger.warning(
                f"Failed to update expansion for character {char_id} ({char_name})"
            )
            return False
    else:
...
def generate_expansions(
    ...
    if args.manual:
        ...
        target_character_section = prepare_character_from_json(
            manual_context_data, target_character["id"]
        )
    ...
    prompt = build_prompt(prompt_data, target_character_section, context)
    ...
    try:
        ...
        logger.info(
            f"Input tokens: {llm_response.input_tokens}, Output tokens: {llm_response.output_tokens}"
        )
        logger.info(f"Generated expansion for character ID {result.character_id}")

        # Process the result
        updated = process_character_expansion(
            result, target_character, engine, args.dry_run, args.force, args.output
        )
...
def main():
    ...
    try:
        ...
        if args.input:
            ...
            if not target_character:
                logger.warning(
                    f"No matching character found in database for ID {char_id}"
                )
                return 1

            # Process the loaded character expansion
            success = process_character_expansion(
                character_expansion, target_character, engine, args.dry_run, args.force
            )

            logger.info(
                f"Successfully imported and updated character ID {char_id}"
                if success
                else f"Failed to update character ID {char_id}"
            )
        ...
        logger.info(
            f"Processing character ID: {target_character['id']}, Name: {target_character['name']}"
        )
        ...
        if args.model.startswith("o"):
            logger.info(
                f"Using reasoning model: {args.model} with effort: {args.effort}"
            )
            ...
            logger.info(
                f"Using standard model: {args.model} with temperature: {args.temperature}"
            )
            api_provider = OpenAIProvider(
        ...
        success_count, total_count = generate_expansions(
            api_provider, target_character, all_characters, engine, args
        )
